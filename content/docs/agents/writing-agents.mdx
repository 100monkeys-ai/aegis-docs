...
The references appear in the prompt context as `{{attachments}}` and in `/workspace/params.json`. Each file is read on demand with `aegis.attachment.read({volume_id, path})`.
