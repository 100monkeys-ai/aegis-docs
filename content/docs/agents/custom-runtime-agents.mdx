...
For most use cases, use the manifest-first default guide: [Writing Your First Agent](/docs/agents/writing-agents).
...
<Callout type="info">
`podman build` uses the same Dockerfile format and accepts the same flags. Replace `docker` with `podman` in the commands above if using Podman as your container engine.
</Callout>
...
<Callout type="info">
**Podman host resolution** -- If running locally under Podman, the hostname `host.docker.internal` may not resolve. Use `host.containers.internal` instead, or set the `AEGIS_ORCHESTRATOR_URL` environment variable explicitly. In production, the orchestrator injects this variable automatically at container start, so the default value in the script is only used during local development.
</Callout>
...
The custom bootstrap must implement the dispatch protocol to communicate with the orchestrator via `AEGIS_ORCHESTRATOR_URL`. The orchestrator sets this variable at container start to the correct host address for the active runtime (Docker or Podman).
...
- [Writing Your First Agent](/docs/agents/writing-agents) — Standard (manifest-only) agents
...
- [Deploying Agents](/docs/agents/deploying-agents)
