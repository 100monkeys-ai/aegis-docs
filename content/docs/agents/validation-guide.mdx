...
Operators can explicitly bypass this semantic gate with `skip_judge` for approved tools. That removes only the judge step; the call still remains subject to the normal policy and authorization path. For the complete gate semantics and bypass rules, see [Tool-Call Judging](/docs/agents/tool-call-judging).
