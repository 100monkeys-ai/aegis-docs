...
When a dispatch carries an `attachments` array (e.g. from a Zaru chat upload, an SDK call, or an enclosing workflow state), the orchestrator surfaces those references in three places inside the running container, mirroring the standard input hydration pipeline:

| Surface | Where it appears |
|---|---|
| Handlebars context | `{{attachments}}` resolves to the array; individual entries are addressable via `{{attachments.[0].name}}` etc. |
