...
See [Configuring Agent Validation](/docs/agents/validation-guide) for full configuration details.

---

## Workflow Execution Output

When a workflow declares `output_schema` and `output_template` in its metadata, the engine produces a structured `final_output` at completion. The lifecycle is:

1. **States complete** -- each state writes its output to the Blackboard as usual.
2. **Blackboard populated** -- by the time the workflow reaches a terminal state, the Blackboard contains all state outputs.
3. **Output template evaluated** -- the engine renders each `output_template` value as a Handlebars expression against the final Blackboard. Each leaf value is rendered independently.
4. **Type coercion** -- rendered string values are coerced to the types declared in `output_schema` (e.g. `parseInt` for integers, `parseFloat` for numbers, `=== "true"` for booleans, `JSON.parse` for objects and arrays).
5. **Validation** -- the coerced output is validated against `output_schema`. If coercion or validation fails, the workflow fails with a descriptive error identifying the field and the value that could not be coerced or validated.
6. **Storage** -- the validated output is stored as `WorkflowExecution.final_output` in PostgreSQL and returned as `last_output` in `aegis.workflow.wait` tool responses.

Workflows without `output_template` complete as before -- the Blackboard is captured but no structured `final_output` is produced.

---

## Attachments Propagation

When a dispatch carries an `attachments` array (e.g. from a Zaru chat upload, an SDK call, or an enclosing workflow state), the orchestrator merges those references into the agent's JSON `input` under the key `attachments` before the prompt is built. If the caller's `input` is a JSON object, `attachments` is added as a sibling key. If the caller's `input` is a scalar (string, number) and attachments are non-empty, the orchestrator wraps it as `{ "value": <scalar>, "attachments": [...] }`. The merged input is what the agent's LLM sees when `{{input}}` is substituted into the agent's prompt template.

The same merged input also reaches the running container through the standard hydration surfaces:

| Surface | Where it appears |
|---|---|
| LLM prompt | The substituted `{{input}}` JSON contains `input.attachments`. The agent's `task.instruction` references `input.attachments` in prose. |
| Environment | `INTENT_INPUTS` JSON contains `params.input.attachments`. |
| Filesystem | `/workspace/params.json` includes `attachments` in its `input` object. |

Each `AttachmentRef` is `{ volume_id, path, name, mime_type, size, sha256? }`. The agent reads the actual bytes by calling `aegis.attachment.read({ volume_id, path })`, which returns base64 content plus MIME, size, and SHA-256. The tool is read-only and tenant-scoped. Agents that consume attachments must declare them in `spec.input_schema`, include `aegis.attachment.read` in `spec.tools`, and be `type: system`.

> Agent `task.instruction` is not re-rendered as a Handlebars template at runtime — the orchestrator substitutes `{{instruction}}` into `prompt_template` once, and the result is passed to the LLM as raw text. To loop over attachments, the instruction describes the loop in prose (e.g. "for each entry in `input.attachments`, call `aegis.attachment.read({volume_id, path})`"); embedding `{{#each attachments}}` in the instruction body would reach the LLM as literal text and is rejected by the agent generator's validation.
