...
Container step outputs land on the [Blackboard](/docs/workflows/overview#the-blackboard) under the state name, accessible by subsequent states:
...
```bash
aegis workflow deploy ./build-pipeline.yaml --force
aegis workflow run build-pipeline --input '{"task": "Write a Rust CLI tool that counts lines in a file"}'
```

Use `--force` when you are replacing an existing pipeline definition with the same name and version.
...
- [Configuring Storage](/docs/deployment/configuring-storage) — Volume types, lifecycle, and access modes
- [Writing Agents](/docs/agents/writing-agents) — Build the agent steps that feed into your CI/CD pipeline
- [Human Approvals](/docs/workflows/human-approvals) — Add operator approval gates between pipeline stages
- [Building Swarms](/docs/workflows/building-swarms) — Spawn child agents from within a running workflow
