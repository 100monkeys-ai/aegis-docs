...
| **Custom container** | Advanced use case for complex dependencies or a custom `bootstrap.py`. See [Custom Runtime Agents (Advanced)](/docs/guides/custom-runtime-agents). |
...
Filter-by-status output is not yet exposed as a CLI flag; filter the table output in your shell if needed.
...
The current CLI does not expose a general `--output json` mode. For shell automation, capture the UUIDs printed by the CLI and poll with the status subcommand:

```bash
# Deploy and capture the agent ID from stdout
AGENT_ID=$(aegis agent deploy ./agent.yaml 2>&1 | grep -oE '[0-9a-f-]{36}' | tail -1)
...
# Poll until complete
while true; do
  STATUS=$(aegis task status "$EXEC_ID" | awk -F': ' '/Status:/ {print tolower($2)}')
  echo "Status: $STATUS"
