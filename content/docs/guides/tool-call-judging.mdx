...
That distinction matters in production. A pre-execution judge can stop a dangerous write, delete, or external request before it reaches a side effect boundary. It is a safety and reliability layer, not a replacement for SMCP, policy enforcement, or argument validation.
...
`skip_judge` does not bypass SMCP signature verification, SecurityContext policy, tool argument validation, or routing. It only removes the inner-loop semantic pre-execution review for the marked tool.
...
In enterprise deployments, treat the tool-call judge as a semantic control layer that complements hard policy enforcement. SMCP still enforces identity and capability boundaries; the judge adds intent-level review before action.
