...
- Run as **child executions** in the execution tree — viewable via `aegis execution list --parent <execution-id>`.
...
The judge runtime reads the code under review from the shared workspace, evaluates it, and writes the JSON verdict to `/workspace/verdict.json`.
