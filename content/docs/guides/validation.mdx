...
Unlike outer validators, the inner `tool_validation` acts synchronously:
1. Agent decides to call a tool (e.g. `cmd.run`).
2. Orchestrator intercepts the call and runs the `tool_validation` judge.
3. If the judge returns confidence < `threshold`, the tool call is rejected, an error is appended to the inner-loop prompt, and the agent tries again *without* terminating the full iteration.
...
- Inherit the worker execution's mounted volumes as read-only views at the same `/workspace...` mount paths, so file-based checks run against the exact worker artifacts.
...
The judge runtime reads the code under review from the inherited `/workspace...` mounts, evaluates it, and writes the JSON verdict to `/workspace/verdict.json`.
