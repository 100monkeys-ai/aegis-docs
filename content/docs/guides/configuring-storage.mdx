...
| **Persistent** | CLI or API | No automatic cleanup; must be explicitly deleted via `aegis volume delete` |
...
VOL_ID=$(aegis volume list --output json | jq -r '[.[] | select(.name=="output")][0].id')
