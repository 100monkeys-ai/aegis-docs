...
| `mount_path` | string | ✓ | Absolute path inside the container where the volume appears. |
| `access_mode` | `ReadOnly` \| `ReadWrite` | ✓ | Access mode enforced by the storage gateway. |
| `storage_class` | `ephemeral` \| `persistent` | ✓ | Lifetime of the volume. |
| `ttl` | duration string | Required for ephemeral | Auto-deletion TTL. Supports `m` (minutes), `h` (hours), `d` (days). |
| `volume_id` | string | | Pin to a specific existing volume. If omitted, a new volume is created. |
| `size_limit_bytes` | integer | | Maximum allowed size. Writes that exceed this emit a `VolumeQuotaExceeded` event and return an error. |
...
- **Filesystem policy enforcement**: Read/write path allowlists from `spec.security.filesystem_policy` are enforced per-operation.
- **Quota enforcement**: Writes that would exceed `size_limit_bytes` are blocked and emit `VolumeQuotaExceeded`.
...
volumes:
  - name: workspace
    ...
    storage_class: ephemeral
    ttl: 2h
    size_limit_bytes: 5368709120   # 5 GiB
```
...
**Single-writer constraint**: In Phase 1, a persistent volume with `ReadWrite` access can only be mounted by one execution at a time. Attempting to mount an already-in-use `ReadWrite` persistent volume will cause the second execution to fail at startup with a `VolumeAlreadyMounted` error.
