   ...
   +-- 2. pod-secrets (OpenBao)      <- Core needs secrets
       +-- 3. pod-iam (Keycloak)     <- Needs PostgreSQL
           +-- 4. pod-core           <- Needs DB, secrets, optionally IAM
               ...
               +-- 7. pod-smcp-gateway <- Needs DB, Podman socket
...
# 3. SMCP keys not generated
ls -la /path/to/smcp-keys/
...
for pod in core database temporal secrets iam storage observability smcp-gateway; do
