...
# Verify from container perspective (agent containers use aegis-exec-network)
podman run --rm --network aegis-exec-network alpine ping -c1 host.containers.internal
...
```bash
# Check platform pod network connectivity (aegis-network)
...
# DNS resolution within platform pods
...
# Inspect agent execution network (agent containers use this, not aegis-network)
podman network inspect aegis-exec-network
```
...
- [Disaster Recovery](/docs/deployment/disaster-recovery) — recovery from failures
