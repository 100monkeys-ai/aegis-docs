...
BUILD:
  ...
  registry_credentials: "vault:cicd/ghcr-token"
...
Store the registry credential in the secrets vault and reference it with the `vault:` prefix. The orchestrator resolves the credential at runtime and never exposes it to the container process environment or logs.
