...
> **Implementation status:** `spec.advanced.bootstrap_path` is declared in the manifest schema and exposed in both the Python SDK (`ManifestBuilder.with_bootstrap_path()`) and TypeScript SDK (`ManifestBuilder.withBootstrapPath()`). The runtime wiring that actually uses this path at container execution time is tracked in the orchestrator backlog and is not yet active in the current release.
