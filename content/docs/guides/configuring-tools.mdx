...
spec:
  mcp_servers:
    - name: web-search
      ...
      # Values support env: (host env var) and vault: (OpenBao KV path).
...
mcp_servers:
  ...
  - name: gmail-tools
    ...
    credentials:
      GMAIL_OAUTH_TOKEN:   "vault:aegis-system/tools/gmail-oauth-token"
      GMAIL_CLIENT_SECRET: "vault:aegis-system/tools/gmail-client-secret"
    health_check:
...
The orchestrator resolves `env:` and `vault:` references in `credentials` at daemon startup using the same credential chain as all other secrets:

| Prefix | Resolution |
|---|---|
| `env:VAR_NAME` | Read from orchestrator process environment at startup |
| `vault:path/to/secret` | Fetched from OpenBao KV engine at startup; cached with a 30-second TTL |
...
| **Credentials** | None injected | Injected from `env:` / `vault:` at startup |
