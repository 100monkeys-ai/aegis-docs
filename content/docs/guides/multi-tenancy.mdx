...
| **2. Authorization** | SMCP Contexts | `SecurityContext` names are namespaced (e.g., `tenant-acme-corp-research`). |
...
## Layer 2: Authorization (SMCP SecurityContext)
...
Tenant provisioning is handled by the `TenantOnboardingService` (triggered via the Control Plane or CLI). The sequence is atomic:
