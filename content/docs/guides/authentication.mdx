...
Issued via `POST /api/edge/enrollment-tokens` with the user's existing JWT in the `Authorization` header. The endpoint requires the `aegis.edge.enroll` role. The token is a short-lived JWT (default ≤15 minutes) signed by the same OpenBao Transit key that signs `NodeSecurityToken`. Its claims:
...
For the operational guide, see [edge host setup](/docs/guides/edge-host-setup); for the security model, see [edge security](/docs/concepts/edge-security).
