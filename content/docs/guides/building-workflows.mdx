...
## Workflow Metadata: description and tags

Every workflow manifest should include `metadata.description` and `metadata.tags`.
...
The `tags` field is a free-form string array of lowercase hyphenated capability descriptors. Use 3
to 5 tags that classify the workflow's domain (e.g., `ci-cd`, `data-processing`, `code-review`,
`kubernetes-deployment`, `api-integration`). Tags are indexed for filtering in
`aegis.workflow.search` and `aegis.workflow.list` responses, and they are included in the embedding
text to improve semantic search recall.
...
|-------|----------------|
| `user` | Only the user who registered the workflow |
...
unless you want `user` scope:

```yaml
metadata:
  name: my-pipeline
  version: "1.0.0"
  scope: user   # omit to default to tenant scope
```

**Global scope is reserved for platform builtins** such as `builtin-intent-to-execution`. Promoting
or demoting a workflow's scope is an admin operation.

When referencing a workflow by name — for example in `aegis workflow run <name>` or an
`aegis.workflow.run` tool call — the system resolves using narrowest-first order: user-scoped
workflows first, then tenant-scoped, then global builtins. A tenant-scoped workflow named
`my-pipeline` will be found before a global workflow of the same name.
