...
Discovery is an enterprise feature. If the node does not have the `discovery` section configured, these tools return an error. Use `aegis.agent.list` and `aegis.workflow.list` as the universal fallback.
