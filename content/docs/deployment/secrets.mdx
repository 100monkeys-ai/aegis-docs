...
AEGIS Daemon (Orchestrator)
  ...
  ├── Issues SEAL SecurityToken JWTs via Transit Engine
...
- **SEAL SecurityToken signing**: The orchestrator signs agent JWTs using a Transit key, so the private key never leaves OpenBao memory.
...
# Create a signing key for SEAL tokens
bao write transit/keys/seal-signing-key type=ed25519
...
```
OpenBao Namespaces:
  aegis-system/     ← platform-level secrets (LLM keys, SEAL keys)
...
path "transit/sign/seal-signing-key" {
  capabilities = ["update"]
}
