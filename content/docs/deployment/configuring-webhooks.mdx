...
Or use the `spec.billing.stripe_webhook_secret` field in `aegis-config.yaml` (see [Configuration Reference](/docs/deployment/configuration)).
...
See [Zaru Overview — Subscription Billing](/docs/zaru/overview#subscription-billing) for the full tier lifecycle.
...
- [Building Workflows](/docs/workflows/building-workflows) — define workflows to handle incoming stimuli
- [REST API — `POST /v1/webhooks/{source}`](/docs/reference/rest-api#post-v1webhookssource)
- [REST API — Billing](/docs/reference/rest-api#billing) — checkout, portal, subscription, and invoice endpoints
