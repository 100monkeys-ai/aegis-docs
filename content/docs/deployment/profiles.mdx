...
- No SMCP Gateway (direct tool routing only)
...
The `full` profile starts every public service including distributed storage (SeaweedFS) and the SMCP tooling gateway. This is the recommended profile for production deployments.
...
| `KEYCLOAK_ADMIN_PASSWORD` | `changeme` | Strong password |
