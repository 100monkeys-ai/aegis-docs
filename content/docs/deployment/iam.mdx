...
description: Keycloak realms, AegisRole claims, M2M service accounts, SAML federation, JWKS cache, and KeycloakAuthInterceptor.
---

# IAM & Identity Federation

AEGIS uses **Keycloak** as its OIDC identity provider. Keycloak issues JWTs for all human operators and machine-to-machine (M2M) clients. The orchestrator validates these JWTs on every API request via the `KeycloakAuthInterceptor`.
...
Use the returned `access_token` as a Bearer token on all AEGIS API requests:

```bash
aegis agent list --token $ACCESS_TOKEN
# or set in environment:
export AEGIS_TOKEN=$ACCESS_TOKEN
aegis agent list
```
...
The `KeycloakAuthInterceptor` maintains a per-realm JWKS cache (fetched from `https://{keycloak}/realms/{realm}/protocol/openid-connect/certs`).

Default cache TTL is 300 seconds (configurable via `iam.keycloak.jwks_cache_ttl`). When Keycloak rotates signing keys, the cache is refreshed automatically on the next TTL expiry. Key rotation completes without any AEGIS downtime.
