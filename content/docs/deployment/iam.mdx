...
Edge daemons and the Relay Coordinator add a small set of Keycloak entities and roles. These are seeded by the standard bootstrap scripts (`bootstrap-keycloak.sh`, `bootstrap-relay-coordinator.sh`) and surface in the standard role-based authorization flow above.

### Service accounts

| Client              | Purpose                                                                                                  | Realm          |
| ------------------- | -------------------------------------------------------------------------------------------------------- | -------------- |
| `relay-coordinator` | The Relay Coordinator service-account client; obtains tokens for edge enrollment and routing operations. | `aegis-system` |

### Roles

| Role                      | Granted to                                         | Required for                                                                                                     |
| ------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `aegis.edge.enroll`       | Users / SAs that may issue enrollment tokens       | `POST /api/edge/enrollment-tokens`                                                                               |
| `aegis.edge.heartbeat`    | Edge daemons (via their `NodeSecurityToken`)       | Heartbeat / capability-update RPCs                                                                               |
| `aegis.edge.hosts.read`   | Operators reading host lists                       | `GET /api/edge/hosts*`                                                                                           |
| `aegis.edge.hosts.write`  | Operators editing tags / revoking hosts            | `PATCH`/`DELETE` on `/api/edge/hosts*`                                                                           |
| `aegis.edge.groups.write` | Operators managing groups                          | `POST`/`PATCH`/`DELETE` on `/api/edge/groups*`                                                                   |
| `aegis.edge.fleet.read`   | Operators previewing targets / reading run history | `POST /api/edge/fleet/preview`, `GET /api/edge/fleet/runs`                                                       |
| `aegis.edge.fleet.manage` | Operators dispatching and cancelling fleet runs    | `POST /api/edge/fleet/invoke`, `POST /api/edge/fleet/:id/cancel`, the `aegis.edge.fleet.*` system-tier MCP tools |

### Tenant resolution

Every endpoint and every fleet operation is filtered through `effective_tenant`. Service accounts using `X-Tenant-Id` resolve to the delegated tenant; cross-tenant access is impossible at the router boundary regardless of how the target is expressed. See [multi-tenancy](/docs/deployment/multi-tenancy) for the broader tenant isolation guarantees.
