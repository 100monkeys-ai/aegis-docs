...
| ------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `aegis.edge.enroll`       | Users / SAs that may issue enrollment tokens       | `POST /v1/edge/enrollment-tokens`                                                                               |
| `aegis.edge.heartbeat`    | Edge daemons (via their `NodeSecurityToken`)       | Heartbeat / capability-update RPCs                                                                               |
| `aegis.edge.hosts.read`   | Operators reading host lists                       | `GET /v1/edge/hosts*`                                                                                           |
| `aegis.edge.hosts.write`  | Operators editing tags / revoking hosts            | `PATCH`/`DELETE` on `/v1/edge/hosts*`                                                                           |
| `aegis.edge.groups.write` | Operators managing groups                          | `POST`/`PATCH`/`DELETE` on `/v1/edge/groups*`                                                                   |
| `aegis.edge.fleet.read`   | Operators previewing targets / reading run history | `POST /v1/edge/fleet/preview`, `GET /v1/edge/fleet/runs`                                                       |
| `aegis.edge.fleet.manage` | Operators dispatching and cancelling fleet runs    | `POST /v1/edge/fleet/invoke`, `POST /v1/edge/fleet/:id/cancel`, the `aegis.edge.fleet.*` system-tier MCP tools |
