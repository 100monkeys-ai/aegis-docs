...
This is distinct from consumer identity resolution, where a `zaru-consumer` realm JWT with a `zaru_tier` claim maps to `IdentityKind::ZaruUser` and a tier-specific SecurityContext (`zaru-free`, `zaru-pro`, etc.).
