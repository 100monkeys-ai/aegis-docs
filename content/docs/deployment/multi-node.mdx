...
## Per-Node FUSE Daemon

In rootless Podman deployments, each node runs its own FUSE daemon instance. The FUSE daemon is a host-local process -- it connects to the local orchestrator's gRPC endpoint and manages FUSE mountpoints on the local filesystem only.
...
│       Node A             │    │       Node B             │
│                          │    │                          │
│  aegis-fuse-daemon       │    │  aegis-fuse-daemon       │
│    → grpc://localhost:50051  │    │    → grpc://localhost:50051  │
│    → /tmp/aegis-fuse/    │    │    → /tmp/aegis-fuse/    │
│                          │    │                          │
│  aegis-orchestrator      │    │  aegis-orchestrator      │
│    → calls local FUSE    │    │    → calls local FUSE    │
│      daemon for mounts   │    │      daemon for mounts   │
└──────────────────────────┘    └──────────────────────────┘
```

Each FUSE daemon is configured independently via the node's `aegis-config.yaml`:

```yaml
storage:
  fuse_mount_prefix: "/tmp/aegis-fuse"
  fuse_daemon_address: "127.0.0.1:50052"
```

The FUSE daemon does not participate in cluster coordination -- it is purely a local mount management service. Cross-node volume access is handled by the SEAL remote storage protocol, not by FUSE.
...
## Edge Node Distinction

Edge daemons are a **separate role** from Controller / Worker / Hybrid and are **not** orchestrator peers. Specifically:

- They are not selected by `NodeCluster::healthy_workers()`.
- They do not receive `ForwardExecution` calls.
- They do not participate in cross-worker storage or volume coordination.
- Their tenant binding is **immutable** post-enrollment, unlike workers which are tenant-agnostic.

What they _do_ is accept MCP tool calls dispatched specifically to them via the `EdgeRouter`, executing those calls locally on the user's host. They open a long-lived bidirectional gRPC stream **outbound** to either the cluster controller (in OSS deployments) or the **Relay Coordinator** at `relay.example.com:443` (in SaaS deployments).

| Aspect                       | Worker                                                | Edge daemon                                                 |
| ---------------------------- | ----------------------------------------------------- | ----------------------------------------------------------- |
| Binding model                | Tenant-agnostic; cluster member                       | Tenant-bound; one daemon per tenant                         |
| Connection direction         | Worker dials controller `:50056`                      | Edge dials controller `:50056` (OSS) or `:443` (SaaS Relay) |
| Receives `ForwardExecution`? | Yes                                                   | No                                                          |
| Receives `InvokeTool`?       | No (workers serve via the orchestrator's tool router) | Yes (via `EdgeRouter`, with two-envelope SEAL)              |
| Inbound port                 | `50057` (FUSE) optionally                             | None                                                        |
| Membership pool              | `healthy_workers()`                                   | `edges_for_tenant(tenant_id)`                               |

For the cluster-extension role used in SaaS, see [edge relay deployment](/docs/deployment/edge-relay-deployment); for the daemon side, see [edge daemon installation](/docs/deployment/edge-daemon-installation).
