...
When an agent executing on Worker Node A needs to access a volume that resides on Worker Node B, AEGIS transparently proxies the file operation via the **RemoteStorageService** gRPC protocol (ADR-064).
