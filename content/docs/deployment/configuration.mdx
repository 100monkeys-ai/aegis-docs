...
- **Node Identity**: Each node maintains a stable identity via a persistent Ed25519 keypair, used for secure attestation and SmcpNodeEnvelope signing.
- **Secure Communication**: Inter-node traffic is protected by mTLS and SMCP-derived security tokens.
...
discovery:
  # Qdrant gRPC endpoint URL. Supports env: prefix.
  qdrant_url: "env:QDRANT_URL"
  # Embedding service gRPC endpoint URL. Supports env: prefix.
  embedding_url: "env:EMBEDDING_SERVICE_URL"
  # Similarity threshold for generation dedup. Default: 0.85
  deduplication_threshold: 0.85
  # Index existing resources at startup. Default: true
...
Optional. Enterprise feature. Enables semantic search over agents and workflows (`aegis.agent.search`, `aegis.workflow.search`) and similarity-based deduplication during generative authoring. Requires a running Qdrant vector store and an embedding service accessible over gRPC. Omit this section entirely to disable discovery.
...
| `deduplication_threshold` | f64 | `0.85` | Similarity score threshold (0.0–1.0) for detecting near-duplicate agents or workflows during generation. |
| `backfill_on_startup` | bool | `true` | Index all existing agents and workflows into Qdrant at startup. |
