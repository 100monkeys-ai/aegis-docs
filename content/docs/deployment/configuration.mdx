...
| `vault:path/to/secret` | Resolved from OpenBao at runtime (requires `spec.secrets.openbao` configured). |
| *(literal)* | Plaintext. Avoid for secrets. |

The recommended production pattern is `vault:` references for all API keys and credentials. Use `env:` as a fallback when OpenBao is not available.
...
llm_providers:
  - name: openai-primary           # Unique provider name on this node. Required.
    ...
    api_key: "env:OPENAI_API_KEY"  # Supports env: and vault: prefixes.
...
storage:
  # Storage backend. Options: seaweedfs | local. Default: local
  backend: seaweedfs
  ...
  # Local storage config. Used when backend: local, or as the fallback target.
  local:
...
mcp_servers:
  - name: web-search              # Unique server name on this node. Required.
    ...
    capabilities:
      - web.search
      - web.fetch
    # API keys and tokens — resolved via env: or vault: prefixes.
...
# Referenced by name in agent manifests and in spec.keycloak ZaruTier mappings.
security_contexts:
  - name: coder-default
...
# ─── gRPC Auth ────────────────────────────────────────────────────────────────
# Optional. Controls Keycloak JWT enforcement on the gRPC endpoint.
# Requires spec.keycloak to be configured.
grpc_auth:
...
# OpenBao — agents never receive vault credentials directly.
secrets:
  # Secrets backend. Options: openbao | environment. Default: environment
  backend: "openbao"
  openbao:
    ...
    # Background token renewal settings.
    token_renewal:
      # Renewal check interval in seconds. Default: 1800 (30 min)
      interval_seconds: 1800
      # Renew when this many seconds remain before expiry. Default: 300
...
database:
  # PostgreSQL connection URL. Supports env: and vault: prefixes.
...
| `api_key` | string | ❌ | null | API key. Supports `env:` and `vault:`. |
...
| `orchestrator_url` | string | `http://localhost:8000` | Callback URL reachable from inside agent containers |
...
| `port` | u16 | `8000` | HTTP REST API port |
...
| `credentials` | map | `{}` | API keys/tokens injected as env vars. Values support `vault:`. |
...
| `command_allowlist` | string[] | Allowed shell base commands (for `cmd.run`) |
...
Optional. Configures Keycloak as the trusted OIDC issuer. Omit to disable JWT validation (dev only; never in production).
...
Optional. Controls Keycloak JWT enforcement on the gRPC endpoint. Requires `spec.keycloak` to be configured.
...
| `openbao.auth_method` | string | `approle` | Authentication method. Only `approle` is currently supported. |
| `openbao.approle.role_id` | string | — | AppRole Role ID (public; safe to commit) |
| `openbao.approle.secret_id_env_var` | string | — | Name of the environment variable containing the AppRole Secret ID. Never commit the actual Secret ID. |
...
| `worker_secret` | string | null | Shared secret for authenticating worker callbacks. Supports `env:` and `vault:`. |
