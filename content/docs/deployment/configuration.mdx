...
metadata:
  name: "my-aegis-node"
spec:
  # All configuration keys documented below sit under spec:
  # llm_providers, llm_selection, runtime, storage, network, ...
```

The sections in the full annotated configuration below all belong under the top-level `spec:` key.
...
server:
  ...
  # Address for the internal gRPC API (Temporal worker / inner-loop transport).
