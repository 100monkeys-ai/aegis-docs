...
# ─── Stripe Billing ──────────────────────────────────────────────────────────
# Optional. Omit all Stripe variables for self-hosted deployments where
# billing is not required. When STRIPE_SECRET_KEY is absent, all /v1/billing/*
# endpoints return 501 Not Implemented.
stripe:
  # Stripe API secret key (server-side only). Supports env: and secret: prefixes.
  secret_key: "env:STRIPE_SECRET_KEY"
  # Stripe publishable key (passed to frontend for Checkout and Pricing Table).
  ...
  # Stripe embedded pricing table component ID (used by the Zaru frontend).
...
Optional. Configures Stripe billing integration. Omit all Stripe variables for self-hosted deployments where billing is not required. When `secret_key` is absent, all `/v1/billing/*` endpoints return `501 Not Implemented`.

| Key | Type | Required | Default | Description |
|-----|------|----------|---------|-------------|
| `secret_key` | string | Yes | — | Stripe API secret key (server-side only). Supports `env:` and `secret:` prefixes. |
| `publishable_key` | string | Yes | — | Stripe publishable key, passed to the frontend for Checkout and Pricing Table rendering. |
| `webhook_secret` | string | Yes | — | Webhook signing secret for verifying Stripe event signatures (HMAC-SHA256). |
| `prices.pro_monthly` | string | Yes | — | Stripe Price ID for Pro tier, monthly billing. |
| `prices.pro_annual` | string | Yes | — | Stripe Price ID for Pro tier, annual billing. |
| `prices.business_monthly` | string | Yes | — | Stripe Price ID for Business tier, monthly billing. |
| `prices.business_annual` | string | Yes | — | Stripe Price ID for Business tier, annual billing. |
| `prices.enterprise_monthly` | string | Yes | — | Stripe Price ID for Enterprise tier, monthly billing. |
| `prices.enterprise_annual` | string | Yes | — | Stripe Price ID for Enterprise tier, annual billing. |
| `pricing_table_id` | string | No | — | Stripe embedded pricing table component ID, used by the Zaru frontend to render plan selection. |
