...
| _(literal)_             | Plaintext. Avoid for secrets.                                                  |
...
runtime:
  ...
  # Container network name for agent containers. Must be a separate network
  # from the platform pods (aegis-network) to prevent container lifecycle
  # churn from disrupting platform pod routing via Podman/netavark.
  # Supports env: prefix. Example: "env:AEGIS_CONTAINER_NETWORK"
  container_network_mode: "aegis-exec-network"
...
network:
  # Bind address for all listeners. Default: 127.0.0.1 (loopback).
  # Binding a non-loopback address requires either `tls` configured below or
  # `allow_insecure_bind: true` (operator-acknowledged opt-out — see below).
  ...
  # Operator-acknowledged opt-out of the non-loopback-without-TLS startup gate.
  # Default: false (fail-closed — pod refuses to start with a non-loopback bind
  # and no TLS).
  #
  # Set true ONLY when TLS is terminated UPSTREAM of this pod by a
  # trusted-network ingress (e.g. Caddy in the same Podman pod-network) AND the
  # bound endpoint is reachable only from inside that trusted network. The
  # validator emits a WARN log on every boot when the override is engaged so the
  # decision is visible in startup logs and audit trails. NEVER set this on a
  # publicly-exposed host — every external request would transit cleartext.
...
storage:
  ...
  # ── FUSE Daemon (rootless Podman deployments) ──────────────────────────────
  # When set, the orchestrator uses the host-side FUSE daemon for agent
  # container volume mounts instead of NFS. Required for rootless Podman.
  # Omit both fields to use NFS (default for rootful Docker).

  # Host directory under which per-volume FUSE mountpoints are created.
  # Default: /tmp/aegis-fuse
  fuse_mount_prefix: "/tmp/aegis-fuse"
  # Address of the external FUSE daemon's FuseMountService gRPC endpoint.
  # This is the gRPC service exposed by the host-side FUSE daemon process,
  # not the internal FSAL (filesystem abstraction) service.
  # When set, enables FUSE transport for agent workspace mounts.
  # When absent, the orchestrator falls back to in-process FUSE, which requires
  # the orchestrator itself to run on the host with /dev/fuse access (not
  # suitable for rootless Podman deployments).
  # Default: null (in-process FUSE / NFS mode)
  fuse_daemon_address: "127.0.0.1:50053"
...
# ─── Billing ──────────────────────────────────────────────────────────────────
# Optional. Omit for self-hosted deployments where billing is not required.
# When stripe_secret_key is absent, all /v1/billing/* endpoints return
# 501 Not Implemented.
# Note: STRIPE_PUBLISHABLE_KEY is a frontend-only env var passed directly to
# the zaru-client container — it is NOT part of NodeConfig and does not belong here.
# Pricing is dynamic — prices are fetched live from Stripe via GET /v1/billing/prices.
billing:
  # Stripe API secret key (server-side only). Supports env: and secret: prefixes.
  stripe_secret_key: "env:STRIPE_SECRET_KEY"
  # Webhook signing secret for verifying Stripe webhook event signatures.
...
| `container_socket_path`  | string | _(platform default)_    | Container runtime socket path. Docker: `/var/run/docker.sock`. Podman rootless: `/run/user/<UID>/podman/podman.sock`. Podman system: `/run/podman/podman.sock`. Can be overridden via `CONTAINER_HOST` or `DOCKER_HOST` env vars. |
...
The reference [pod architecture](/docs/deployment/pod-architecture) and [Caddy edge proxy](/docs/deployment/caddy-edge-proxy) deployments use this opt-out because Caddy fronts the backend pods on a private Podman network. Setting `allow_insecure_bind: true` on a publicly-exposed host is a serious security regression.
</Callout>
...
| `cmd.global_env_denylist`          | string[] | _(see sample)_ | Env vars that must never be forwarded to agent subprocesses |
...
## `cluster.edge` — edge daemon configuration

When `cluster.role: edge`, the daemon requires a `cluster.edge` block. The block is populated automatically by `aegis edge enroll`; `tenant_id` is written at enrollment and is **immutable** thereafter.
...
cluster:
  ...
  role: edge
  controller:
    endpoint: relay.example.com:443 # SaaS default; OSS overrides to local controller
    tls: { enabled: true }
...
| Key                                          | Type                 | Default                        | Description                                                                    |
| -------------------------------------------- | -------------------- | ------------------------------ | ------------------------------------------------------------------------------ |
| `cluster.edge.tenant_id`                     | string               | —                              | Immutable post-enrollment. Mirrors the `tid` claim on the `NodeSecurityToken`. |
| `cluster.edge.enrollment_token_path`         | path                 | `~/.aegis/edge/enrollment.jwt` | One-time enrollment token; consumed atomically.                                |
| `cluster.edge.node_security_token_path`      | path                 | `~/.aegis/edge/node.token`     | Persistent `NodeSecurityToken`.                                                |
| `cluster.edge.stream_reconnect_backoff_secs` | `array<int>`         | `[1, 2, 5, 15, 60]`            | Exponential backoff schedule on stream loss.                                   |
...
**Validator behavior:** `role: edge` rejects `cluster_grpc_port` (the daemon does not bind a server) and requires the `cluster.edge` block. `role: relay-coordinator` requires `cluster.cluster_grpc_port` and `cluster.ingress.public_endpoint`.

For the full edge config reference, see [edge config](/docs/reference/edge-config).
