...
| `AEGIS_FUSE_DAEMON_ENDPOINT` | No | Address of the host-side FUSE daemon (e.g. `http://127.0.0.1:50053`); required for rootless Podman FUSE volume mounting |
...
### FUSE Daemon Volume Mounting

In rootless Podman deployments, agent containers receive their workspace volumes via the host-side FUSE daemon rather than NFS. The FUSE daemon runs as a separate process on the host and must be started before the orchestrator.

#### Starting the FUSE Daemon

```bash
# Start the FUSE daemon (manual)
...
For production deployments, run the FUSE daemon as a systemd user service that starts before the orchestrator:

```ini
# ~/.config/systemd/user/aegis-fuse-daemon.service
[Unit]
...
The FUSE daemon **must be running before the orchestrator pod starts**. The systemd `Before=aegis.service` directive enforces this when using systemd. For non-systemd setups, start the FUSE daemon manually and verify it is listening on port 50053 before launching the orchestrator pod.
</Callout>

#### AEGIS Configuration

Point the orchestrator at the FUSE daemon in `aegis-config.yaml` and set the corresponding environment variable in your `.env` file:

```bash
# .env
AEGIS_FUSE_DAEMON_ENDPOINT=http://127.0.0.1:50053
```
...
When `fuse_daemon_address` is set, the orchestrator uses FUSE bind mounts instead of NFS volume driver mounts for all agent containers. The agent container does not require `CAP_SYS_ADMIN` or any elevated capabilities.

#### Mount Flow

1. Execution starts. Orchestrator calls `FuseMountService.Mount` on the FUSE daemon.
2. FUSE daemon creates a mountpoint at `/tmp/aegis-fuse/<volume_id>`.
3. Orchestrator bind-mounts the host path into the container's `/workspace`.
4. All POSIX operations proxy through the FUSE daemon to the orchestrator's AegisFSAL.
5. On execution end, orchestrator calls `FuseMountService.Unmount` to clean up.
...
```bash
# Create the platform network (for pods)
podman network create aegis-network

# Create the agent execution network (isolated from platform pods)
podman network create aegis-exec-network
