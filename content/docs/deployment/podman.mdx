...
description: Podman rootless deployment, systemd socket activation, bollard API compatibility, cgroup v2 delegation, NFS volume mounting, and differences from Docker.
---

# Podman Deployment

Podman is the daemonless, rootless container runtime supported by AEGIS as an alternative to Docker. The orchestrator communicates with Podman via the same **bollard** Rust library used for Docker — Podman exposes a Docker-compatible API over its Unix socket. This page covers rootless Podman deployment, socket configuration, container lifecycle, and the key differences from Docker.
