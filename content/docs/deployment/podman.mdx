...
For production and staging environments, AEGIS deploys as a set of Podman pods defined in the [`aegis-deploy`](https://github.com/100monkeys-ai/aegis-deploy) repository. Each pod groups related containers with shared networking, health checks, and persistent volumes.
...
| **pod-smcp-gateway** | aegis-smcp-gateway | 8089, 50055 | Tool orchestration gateway |
...
| `make generate-keys` | Generate Ed25519 SMCP signing keypair |
...
| `aegis-smcp-gateway-data` | smcp-gateway | SQLite tool database |
...
Both names resolve to the host IP from within agent containers, so NFS mount addresses and SMCP callbacks work regardless of which runtime is in use.
...
The AEGIS daemon enforces network policy at the SMCP layer (per tool call), not via container network rules.
