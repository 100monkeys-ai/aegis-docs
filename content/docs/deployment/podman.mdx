...
For production and staging environments, AEGIS deploys as a set of Podman pods defined in the [`aegis-deploy`](https://github.com/100monkeys-ai/aegis-deploy) repository. Unlike `aegis init` (which is for local testing and evaluation only), `aegis-deploy` provides a complete, production-ready deployment with IAM, secrets management, and TLS. Each pod groups related containers with shared networking, health checks, and persistent volumes.
