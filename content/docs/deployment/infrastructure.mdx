...
| otelcol-contrib | 0.99+   | OTLP fan-out (logs → Loki, traces → Jaeger)                                  | Required                    |
| Jaeger          | 1.55+   | Distributed tracing                                                          | Recommended                 |
...
└───────────────────────────────────────────────────────────────────────┘
        ...
        aegis-exec-network (Podman bridge — agent containers, isolated)
┌───────────────────────────────────────────────────────────────────────┐
│  Agent Containers (spawned by aegis-runtime via bollard)              │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐                     │
│  │ container1 │  │ container2 │  │ container3 │  ...                 │
│  └────────────┘  └────────────┘  └────────────┘                     │
│  Reach orchestrator via host.containers.internal:8088                 │
└───────────────────────────────────────────────────────────────────────┘
```

Agent containers run on `aegis-exec-network`, isolated from platform pods on `aegis-network`. They communicate with the AEGIS orchestrator via `host.containers.internal:8088` (`/v1/dispatch-gateway` over HTTP). They do not have direct access to PostgreSQL, SeaweedFS, OpenBao, or external networks (unless the manifest `network_policy.allowlist` permits specific domains).
...
| Temporal Web UI  | 8082 | Workflow visibility UI                                                       |
...
| 8082   | HTTP     | Temporal UI              | temporal      | Internal / proxied       |
...
| 4317   | gRPC     | otelcol OTLP gRPC        | observability | Internal only            |
...
| 3000   | HTTP     | Zaru Client              | zaru          | Internal / proxied       |
| 3001   | HTTP     | Zaru MCP Server          | zaru          | Internal / proxied       |
| 8180   | HTTP     | Keycloak IAM             | iam           | Internal / proxied       |
| 4318   | HTTP     | otelcol OTLP HTTP        | observability | Internal only            |
| 13133  | HTTP     | otelcol health           | observability | Internal only            |
| 50052  | gRPC     | Cortex Embedding Service | cortex        | Internal only            |
| 6333   | HTTP     | Qdrant Vector DB         | cortex        | Internal only            |
| 6334   | gRPC     | Qdrant gRPC              | cortex        | Internal only            |
...
In addition to controller and worker nodes, AEGIS supports user-installed **edge daemons** that connect outbound from a user's own host to a controller (OSS) or to the **Relay Coordinator** at `relay.example.com:443` (SaaS). Edge daemons are not orchestrator peers — they receive `InvokeTool` commands targeted specifically at them via the `EdgeRouter`.

### Dependency matrix

| Component                 | Required     | Notes                                                                                                    |
| ------------------------- | ------------ | -------------------------------------------------------------------------------------------------------- |
| gRPC client (h2 over TLS) | yes          | Outbound to the controller endpoint baked into the enrollment token's `cep` claim.                       |
| systemd / launchd / NSSM  | yes (one of) | The daemon runs as a user-level service.                                                                 |
| FUSE                      | optional     | Only if local filesystem mounts are part of the daemon's tool surface.                                   |
| OpenBao backend           | optional     | The daemon defaults to a file-backed local secrets store; OpenBao can be wired in via `secrets.backend`. |
| Inbound listening port    | **no**       | The daemon does not bind any inbound port.                                                               |

### Endpoint

| Subdomain           | Purpose                                             | Notes                                                                               |
| ------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `relay.example.com` | Multi-tenant Relay Coordinator (SaaS edge endpoint) | TLS-terminated at Caddy, h2c reverse-proxy to the Relay pod on internal port 50056. |

In OSS deployments edge handling co-locates with the controller — the existing `50056` cluster gRPC port accepts edge enrollment and stream connections alongside worker membership.

For the daemon-side install, see [edge daemon installation](/docs/deployment/edge-daemon-installation); for the Relay-side deployment, see [edge relay deployment](/docs/deployment/edge-relay-deployment).
