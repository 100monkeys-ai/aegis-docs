...
|---|---|---|---|
| Docker Engine | 24.0+ | Agent container runtime (Phase 1) | ✓ (one of Docker or Podman) |
| Podman | 4.0+ | Agent container runtime (rootless alternative to Docker) | ✓ (one of Docker or Podman) |
| Docker Compose | 2.20+ | Dev/staging orchestration (or `podman compose`) | Dev only |
...
Minimum viable deployment (no Temporal, no workflows, OpenBao, or Keycloak) requires only a container runtime (Docker or Podman), PostgreSQL, and SeaweedFS.
...
│                                                               │
│  ┌────────────────────────────────────────────────────────┐   │
│  │  Agent Containers (Docker/Podman bridge network)       │   │
│  │  ┌────────────┐  ┌────────────┐  ┌────────────┐       │   │
...
Agent containers communicate **only** with the AEGIS daemon (via `/v1/dispatch-gateway` over HTTP) and the NFS server (port 2049). They do not have direct access to PostgreSQL, SeaweedFS, OpenBao, Keycloak, or external networks (unless the manifest `network_policy.allowlist` permits specific domains).
