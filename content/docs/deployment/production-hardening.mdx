...
| SMCP keys | Generate unique Ed25519 keypair (`make generate-keys`) | Critical |
...
- **Keycloak**: Configure HTTPS on port 8443 with X.509 certificates
...
| SMCP signing keys | Regenerate with `make generate-keys`, restart pod-core | Annually |
...
| pod-smcp-gateway | 250m | 256Mi | 1000m | 1Gi |
...
- Disable the default `master` realm admin account after creating dedicated admin users
- Enable brute-force detection on all realms
...
2. Configure Keycloak OIDC authentication for Grafana
3. Set up role-based access control for dashboards
...
trivy image ghcr.io/100monkeys-ai/aegis-smcp-gateway:1.2.3
```
