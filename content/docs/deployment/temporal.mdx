...
If `spec.temporal.max_connection_retries` is omitted, the orchestrator retries Temporal client connection setup 30 times before failing startup.
...
# Inspect and watch the execution
...
When `aegis workflow deploy` is run, the Rust orchestrator registers the serialized FSM via its workflow registration API and simultaneously saves the `Workflow` domain aggregate to PostgreSQL.
...
The Temporal UI remains the authoritative raw engine view, while `aegis workflow logs` provides the AEGIS domain-event view with workflow names, states, and iteration metadata.
