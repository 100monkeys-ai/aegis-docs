...
**File locking**: NFS mounts use the `nolock` option. POSIX advisory locks (`flock`, `fcntl`) are not coordinated between agents. For multi-agent coordination on shared files, use the [ResourceLock](/docs/workflows/swarms#resource-locking) mechanism instead.
...
The reader agent sees the writer's files at `/workspace/input`. Because it mounts `read-only`, it is compatible with the [single-writer constraint](#phase-1-constraints) and can run concurrently with other readers on the same volume.
...
### Retrieving Files via `aegis.execution.file`

The `aegis.execution.file` MCP tool provides a higher-level alternative to direct S3 access for retrieving files from completed executions. It supports both text and binary files:

- **Text files** (source code, Markdown, JSON, etc.) are returned inline in the tool result, making them directly consumable by LLMs in agentic workflows.
- **Binary files** (images, PDFs, archives, compiled artifacts, etc.) are returned with a `download_url` that points to the `GET /v1/executions/:id/files/*path` REST endpoint. Client applications can fetch the binary content out-of-band from this URL.

Files retrieved through this tool are read directly from the storage backend — no running execution or active NFS mount is required. Files remain available until the ephemeral volume's TTL expires (default 24 hours after execution completes). Persistent volumes retain their files indefinitely.
...
For current CLI vs API storage operation coverage, see the [CLI Capability Matrix](/docs/reference/cli-capability-matrix).
