...
<Callout type="warn">
Firecracker support is planned for a future release and is not yet available. The Docker and Podman runtimes are the current deployment targets. This page documents the planned Firecracker integration.
</Callout>
...
| Property | Docker Container | Podman Container (rootless) | Firecracker microVM |
|---|---|---|---|
| Kernel isolation | Shared host kernel | Shared host kernel (user namespace) | Dedicated guest kernel per agent |
| Attack surface | Kernel syscall interface (~350 calls) | Reduced (rootless, no daemon) | Minimal API (≈ 50 ops) |
| Boot time | ~100ms | ~100ms | ~125ms |
| Memory overhead | ~5 MiB | ~5 MiB | ~5 MiB |
| Storage transport | NFS (NFSv3 over bridge network) | NFS (NFSv3 over bridge network) | virtio-fs (shared memory, no network) |
| Blast radius on escape | Host compromise possible | Bounded to user namespace | Bounded to single microVM |
| Daemon process | Privileged daemon required | No daemon (fork/exec) | No daemon (VMM per microVM) |
...
trait AgentRuntime: Send + Sync {
    ...
    /// Get the runtime-specific address for the /v1/dispatch-gateway endpoint
...
- `PodmanRuntime` implements `AgentRuntime` using the same Docker-compatible API via the Podman socket (rootless or system).
...
The same host-side FUSE daemon used for rootless Podman deployments supports the virtio-fs transport for Firecracker. The daemon acts as the vhost-user-fs backend, and the guest kernel communicates via the virtio-fs device. This means a single daemon binary handles both FUSE (containers) and virtio-fs (microVMs).
...
microVM (guest kernel)
  /workspace mounted via virtio-fs
         ...
         ▼
FUSE daemon (host) — same binary as container FUSE transport
         │
         │ gRPC
         ▼
AegisFSAL (orchestrator)
  Same security, authorization, audit, and quota logic as NFS and FUSE
         │
         ▼
StorageProvider (SeaweedFS, OpenDAL, etc.)
```

The key architectural advantage: `AegisFSAL` is the same code regardless of transport. Security invariants (path canonicalization, UID/GID squashing, filesystem policy enforcement, quota) are implemented once and reused across NFS (Docker), FUSE (rootless Podman), and virtio-fs (Firecracker).

---

## Network Isolation in Firecracker

Each Firecracker microVM gets its own network interface via a tap device on the host. Outbound traffic from the microVM is routed through the orchestrator proxy (SEAL), not directly to the internet. The tap interface is destroyed when the microVM stops.
