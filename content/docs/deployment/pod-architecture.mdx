...
│  ┌──────────────────────────────┐    ┌──────────────────┐   │
│  │otelcol    :4317 :4318 :13133│    │seal-gateway      │   │
│  │jaeger     :16686             │    │:8089 :50055      │   │
│  │prometheus :9090              │    │                  │   │
│  │grafana    :3300              │    │                  │   │
│  │loki       :3100              │    │                  │   │
│  │promtail   :9080              │    │                  │   │
...
┌─────────────────────────────────────────────────────────────┐
│               aegis-exec-network (bridge)                    │
│         Agent execution containers — isolated                 │
│                                                               │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐             │
│  │ agent-ctr1 │  │ agent-ctr2 │  │ agent-ctr3 │   ...       │
│  │            │  │            │  │            │             │
│  └────────────┘  └────────────┘  └────────────┘             │
...
**Key environment variables:** `RUST_LOG`, `DATABASE_URL`, `AEGIS_CONTAINER_NETWORK` (set to `aegis-exec-network` — the isolated network for agent containers), `AEGIS_ORCHESTRATOR_URL`, `SEAL_PRIVATE_KEY_PATH`, `AEGIS_OTLP_ENDPOINT`, `CORTEX_GRPC_URL`, `AEGIS_FUSE_DAEMON_ENDPOINT` (address of the host-side FUSE daemon used for workspace volume mounting in rootless Podman deployments)
...
| Container | Image | Ports |
|-----------|-------|-------|
| otelcol-contrib | `otel/opentelemetry-collector-contrib:0.99.0` | 4317 (OTLP gRPC), 4318 (OTLP HTTP), 13133 (health) |
| jaeger | `jaegertracing/all-in-one:1.55` | 16686 (UI) |
...
AEGIS uses a three-network topology:

- **`aegis-network`** — Platform pods (pod-core, pod-database, pod-temporal, etc.). All platform pods share this bridge network and communicate via pod DNS names.
- **`aegis-exec-network`** — Agent execution containers spawned by the orchestrator. Isolated from the platform network to prevent container lifecycle churn from disrupting platform pod routing. Agent containers reach the orchestrator via `host.containers.internal:8088`.
- **`zaru-edge`** — Public-facing Caddy reverse proxy (pod-zaru-edge), bridging external traffic to the platform network.

Containers within the same pod communicate via `localhost`. Cross-pod communication on `aegis-network` uses pod DNS names:
...
| pod-core | pod-observability | gRPC :4317 | OTLP trace/log export (to otelcol) |
