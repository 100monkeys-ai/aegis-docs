...
│  ┌──────────────────────────────┐    ┌──────────────────┐   │
│  │jaeger     :16686 :4317 :4318│    │smcp-gateway      │   │
...
**Key environment variables:** `RUST_LOG`, `DATABASE_URL`, `AEGIS_CONTAINER_NETWORK`, `AEGIS_ORCHESTRATOR_URL`, `SMCP_PRIVATE_KEY_PATH`, `AEGIS_OTLP_ENDPOINT`, `CORTEX_GRPC_URL`
...
SMCP tooling gateway for REST workflow macros, API explorer, and ephemeral CLI tools.

| Container | Image | Ports |
|-----------|-------|-------|
| smcp-gateway | `ghcr.io/100monkeys-ai/aegis-smcp-gateway` | 8089 (HTTP), 50055 (gRPC) |

**Health check:** HTTP GET `/` on port 8089 (10s interval)

**Volumes:**

- `aegis-smcp-gateway-data` -> `/app/data` (SQLite database)
...
| `aegis-smcp-gateway-data` | smcp-gateway | `/app/data` | Permanent |
...
| pod-smcp-gateway | pod-database | TCP :5432 | Tool registry |
