...
Clone the orchestrator repository to get the Docker Compose files and example configuration:
...
This starts PostgreSQL, SeaweedFS, and Temporal. Wait ~15 seconds for Temporal to finish initializing:
...
```bash
cp aegis-config.yaml my-aegis-config.yaml
```
...
The repository ships with example agent manifests in `demo-agents/`.
