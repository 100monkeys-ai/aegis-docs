...
| `aegis fuse-daemon` | `start`, `stop`, `status`                                                                                                                                                                    |
...
The `aegis edge` family of subcommands operates on user-installed edge daemons and the Relay Coordinator surface. See [edge CLI reference](/docs/reference/edge-cli) for the full surface.

| Capability                   | CLI                                                      |
| ---------------------------- | -------------------------------------------------------- |
| Daemon enrollment            | `aegis edge enroll <token>`                              |
| Daemon local status          | `aegis edge status`                                      |
| Daemon local revocation      | `aegis edge logout`                                      |
| Service-manager unit install | `aegis edge service install\|uninstall\|reinstall`       |
| List edges (operator)        | `aegis edge ls [--tag] [--label] [--connected]`          |
| Tag mutation                 | `aegis edge tag add\|rm <node-id> <tag>...`              |
| Group CRUD                   | `aegis edge group ls\|create\|set-pinned\|rm`            |
| Fleet target preview         | `aegis edge fleet preview --target <expr>`               |
| Fleet dispatch               | `aegis edge fleet run --target <expr> --tool <name> ...` |
| Fleet cancellation           | `aegis edge fleet cancel <fleet-command-id>`             |
| Fleet history                | `aegis edge fleet runs`                                  |
| Fleet-wide key rotation      | `aegis edge fleet keys rotate --target <expr>`           |
| Daemon key rotation          | `aegis edge keys rotate`                                 |
| Force token refresh          | `aegis edge token refresh`                               |
| Operator-side revocation     | `aegis edge keys revoke-remote <node-id>`                |
