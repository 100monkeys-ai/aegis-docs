...
spec:
  ...
  execution:
    ...
    tool_validation:
      - type: semantic           # Pre-execution semantic judge for tool calls
...
| `mount_path` | string | ✓ | | Absolute path inside the container (e.g., `/workspace`). |
...
| `validation` | object | | | Outer-loop acceptance criteria (end of iteration). |
| `tool_validation` | `object[]` | | | Inner-loop pre-execution semantic evaluation for tool calls. |
