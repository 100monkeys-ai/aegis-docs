...
spec:
  type: user                        # optional; user | system. Default: user
...
| Field | Type | Required | Description |
|---|---|---|---|
| `type` | `"user"` \| `"system"` | | Execution tier. Default: `"user"`. |
...
|-------|------|----------|---------|
| `type` | `"user"` \| `"system"` | No | `"user"` |

Declares the agent's execution tier.

- **`user`** — The agent runs under the caller's security context. This is the default for all user-created agents.
- **`system`** — The agent runs under a privileged system context with access to filesystem operations and platform management tools. Use this for agents that perform infrastructure or orchestration tasks on behalf of the platform.

Omitting this field is equivalent to `type: user`.
