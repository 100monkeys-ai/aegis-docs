...
**Prompt template variables:** `{{instruction}}`, `{{input}}`, `{{iteration_number}}`, `{{previous_error}}`, `{{context}}`.
...
When `input_schema` is declared, callers pass inputs as a JSON object to `--input` (or the equivalent API field). The orchestrator validates the payload against the schema before dispatching the execution.
