...
spec:
  ...
  volumes:
    - name: workspace
      type: seaweedfs          # Default backend; seaweedfs | opendal | hostPath | smcp
...
| `type` | `seaweedfs` \| `opendal` \| `hostPath` \| `smcp` | | `seaweedfs` | The storage backend mapping orchestrating this volume. |
