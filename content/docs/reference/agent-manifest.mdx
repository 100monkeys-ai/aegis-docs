...
When the orchestrator dispatches an execution with `attachments` populated, the references appear in the agent's prompt context (`{{attachments}}`), in `INTENT_INPUTS` as `params.input.attachments`, and in `/workspace/params.json`. The agent calls `aegis.attachment.read({volume_id, path})` once per file it needs to read.
