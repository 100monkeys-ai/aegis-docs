...
To import community skill definitions as agent baselines, use the built-in [`skill-import` workflow](/docs/agents/importing-skills).
...
#### `intent`, `input`, and `attachments`

Every agent invocation accepts three optional fields on the execution request:
...
- **`attachments`** — array of typed file references (`{ volume_id, path, name, mime_type, size, sha256? }`) describing files staged in tenant-scoped volumes. Merged into the agent's JSON `input` under the key `attachments` before prompt rendering, so the LLM reads them via `input.attachments` from the substituted `{{input}}` slot. Also surfaced in `INTENT_INPUTS` and `/workspace/params.json`. Available across every dispatch entry point (CLI, SDKs, MCP tools, REST, gRPC). See the *Attachments example* below.

These are complementary, not alternatives — `intent` steers the LLM with natural language; `input` supplies typed data; `attachments` carries file references the agent reads on demand via `aegis.attachment.read`.
...
**Attachments example.** Agents that process files uploaded via Zaru chat or attached programmatically through the SDK declare an `attachments` field in their input schema and pair it with the `aegis.attachment.read` tool. Such agents must be `type: system` to access `aegis.attachment.read`.
...
When the orchestrator dispatches an execution with `attachments` populated, the references are merged into the agent's JSON `input` under the key `attachments` before the prompt template is rendered. The agent's LLM reads them at `input.attachments` from the substituted `{{input}}` JSON. The same array is also exposed in `INTENT_INPUTS` as `params.input.attachments` and in `/workspace/params.json`. The agent calls `aegis.attachment.read({volume_id, path})` once per file it needs to read.

The agent's `spec.task.instruction` must reference `input.attachments` in prose. The instruction body is not Handlebars-rendered at runtime — `{{instruction}}` is substituted into `prompt_template` as raw text — so iteration syntax such as `{{#each attachments}}` or `{{this.volume_id}}` is rejected by the agent generator's validation and would otherwise reach the LLM as literal text.
...
| `tool_validation` | `ValidatorSpec[]` | | | Ordered inner-loop tool-call validators. Current runtime consumes `semantic` entries for pre-execution judging; see [Tool-Call Judging](/docs/agents/tool-call-judging). |
...
`tool_validation` is an ordered list of validator specifications. In the current runtime, only `semantic` entries are used for tool-call gating; they run before dispatch and can block the call synchronously. See [Tool-Call Judging](/docs/agents/tool-call-judging) for the inner-loop contract.
...
**Benefits:** Flexibility for OS packages, custom dependencies, private registries. See [Custom Runtime Agents](/docs/agents/custom-runtime-agents) for a complete walkthrough.

#### Bootstrap Script

Both paths use the orchestrator-provided bootstrap script for 100monkeys iteration control. To bundle your own bootstrap script in a CustomRuntime image, declare its path in `spec.advanced.bootstrap_path`. See [Custom Runtime Agents — Bootstrap Handling](/docs/agents/custom-runtime-agents#bootstrap-handling).
...
For tool-call judging specifics, including the judge payload and bypass semantics, see [Tool-Call Judging](/docs/agents/tool-call-judging).
...
- [Building Workflows](/docs/workflows/building-workflows) — `ParallelAgents` state type
- [Validation Guide](/docs/agents/validation-guide) — gradient scoring and confidence gating
...
**See:** [Custom Runtime Agents](/docs/agents/custom-runtime-agents) for complete walkthrough.
