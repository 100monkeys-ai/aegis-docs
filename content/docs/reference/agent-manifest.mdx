...
| `output_schema` | JSON Schema object | | Declares the shape of the agent's structured output. See [`spec.output_schema`](#specoutput_schema). |
...
An optional JSON Schema object declaring the shape of the agent's structured output. This is a descriptive contract — it documents what the agent produces so that callers (workflows, other agents, SDK consumers) know the expected output shape.

| Field | Type | Required | Description |
|---|---|---|---|
| `output_schema` | JSON Schema object | | Describes the agent's output shape. Must be a JSON Schema object with `type: object`. |
