...
See [SEAL Architecture](/docs/architecture/seal) for the full attestation flow.

---

### `POST /v1/seal/invoke`

SEAL-wrapped tool invocation. Called by `bootstrap.py` to invoke a tool through the orchestrator proxy. The body is an `SealEnvelope` containing a signed MCP tool call.
...
- [SEAL Architecture](/docs/architecture/seal) — signed tool invocation protocol
