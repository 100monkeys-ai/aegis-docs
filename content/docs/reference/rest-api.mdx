  ...
  "tiers": [
    {
      ...
      "seat_monthly": {
        "price_id": "price_seat_m",
        "amount": 1500,
        "currency": "usd"
      },
      "seat_annual": {
        "price_id": "price_seat_a",
        "amount": 15000,
        "currency": "usd"
      }
    }
...
The `/api/edge/*` namespace covers enrollment, host management, groups, and fleet operations for user-installed edge daemons. Every endpoint is filtered through `effective_tenant`; cross-tenant access is impossible at the router boundary.

| Method   | Path                                       | Purpose                                                                    |
| -------- | ------------------------------------------ | -------------------------------------------------------------------------- |
| `POST`   | `/api/edge/enrollment-tokens`              | Issue a one-time enrollment token.                                         |
| `GET`    | `/api/edge/hosts`                          | List hosts in the tenant (filterable by tag, label, connected).            |
| `GET`    | `/api/edge/hosts/:id`                      | Single host detail with recent command history.                            |
| `PATCH`  | `/api/edge/hosts/:id`                      | Update display name and/or tags.                                           |
| `POST`   | `/api/edge/hosts/:id/tags`                 | Atomic add/remove on the tags array.                                       |
| `DELETE` | `/api/edge/hosts/:id`                      | Revoke an edge host.                                                       |
| `GET`    | `/api/edge/groups`                         | List groups.                                                               |
| `POST`   | `/api/edge/groups`                         | Create a group.                                                            |
| `GET`    | `/api/edge/groups/:id`                     | Single group with resolved members.                                        |
| `PATCH`  | `/api/edge/groups/:id`                     | Update name, selector, pinned members.                                     |
| `DELETE` | `/api/edge/groups/:id`                     | Delete a group.                                                            |
| `POST`   | `/api/edge/fleet/preview`                  | Resolve a target without dispatching.                                      |
| `POST`   | `/api/edge/fleet/invoke`                   | Dispatch a tool against a target with a fleet policy. **Server-streamed.** |
| `POST`   | `/api/edge/fleet/:fleet_command_id/cancel` | Broadcast `Cancel` to in-flight per-node commands.                         |
| `GET`    | `/api/edge/fleet/runs`                     | Fleet run history.                                                         |
| `GET`    | `/api/edge/fleet/runs/:id`                 | Single run with per-node detail.                                           |

Required scopes (mapped from Keycloak roles): `aegis.edge.enroll`, `aegis.edge.hosts.{read,write}`, `aegis.edge.groups.write`, `aegis.edge.fleet.{read,manage}`.

For the full reference, see [edge REST API](/docs/reference/edge-rest-api).
