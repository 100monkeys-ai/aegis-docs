...
description: Complete input and output schemas for all 62 AEGIS MCP tools — 61 built-in orchestrator tools and the Zaru-only zaru.switch_mode tool — with availability by context and Zaru conversation mode.
---
...
| Zaru Chat — operator mode | All 62 tools | No filtering applied |
| AEGIS agent inner loop | All 61 built-in tools | Subject to agent `spec.tools` allowlist and `SecurityContext` |
| External SEAL caller | All 61 built-in tools | Filtered by `SecurityContext` only; no mode concept |
...
All 62 tools are available — no filtering is applied.
...
Read a file from a completed execution's workspace volume. Use this to retrieve output files after an agent or task execution finishes.
...
<Callout type="info">
  ...
  the same file. Returns file contents as text. Works after execution completes, while the volume
  is still available (ephemeral volumes are cleaned up based on TTL).
</Callout>
