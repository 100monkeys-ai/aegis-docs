...
description: Complete input and output schemas for all 86 AEGIS MCP tools — 83 built-in orchestrator tools and the Zaru-only zaru.init, zaru.mode, and zaru.docs tools — with availability by context and Zaru conversation mode.
---
...
Tool count: **87** total — 84 built-in orchestrator tools + 3 Zaru client tools (`zaru.init`, `zaru.mode`, `zaru.docs`).
...
<Callout type="info">
  `zaru.init`, `zaru.mode`, and `zaru.docs` are intercepted by the
  `zaru-mcp-server` and are never forwarded to the AEGIS orchestrator. They are
  only available when using the StreamableHTTP transport and are injected by the
