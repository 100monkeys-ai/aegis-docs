...
description: Complete input and output schemas for all 63 AEGIS MCP tools — 61 built-in orchestrator tools and the Zaru-only zaru.init and zaru.mode tools — with availability by context and Zaru conversation mode.
---
...
`aegis.schema.get`, `aegis.schema.validate`, `aegis.runtime.list`, `zaru.init`, `zaru.mode`
...
|---------|----------------------|-------|
| Zaru Chat — chat mode | `zaru.init` + `zaru.mode` only | Mode switching only; no orchestrator tools |
| Zaru Chat — agentic mode | `zaru.init` + `zaru.mode` + agent/task management subset | See mode table below |
| Zaru Chat — execute mode | `zaru.init` + `zaru.mode` + execute pipeline tools | See mode table below |
| Zaru Chat — workflow mode | `zaru.init` + `zaru.mode` + workflow/agent design subset | See mode table below |
| Zaru Chat — operator mode | All 63 tools | No filtering applied |
...
Only `zaru.init` and `zaru.mode` are exposed. The model cannot invoke any orchestrator tools directly from chat mode.
...
All 63 tools are available — no filtering is applied.
...
<Callout type="info">
  `zaru.init` and `zaru.mode` are intercepted by the `zaru-mcp-server` and are never forwarded to
  the AEGIS orchestrator. They are only available when using the StreamableHTTP transport and are
  injected by the Zaru MCP server at the point of tool advertisement.
</Callout>
