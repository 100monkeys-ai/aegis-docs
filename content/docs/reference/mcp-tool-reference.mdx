...
<Callout type="info">
  Volume, file, git, and script tools are available to every tier — Free, Pro, Business, and
  Enterprise. The backend enforces per-tier storage, bandwidth, and count quotas; the
  tool-availability gate itself is uniform across tiers.
</Callout>
...
<Callout type="warn">
  Deletes the binding and cascade-deletes the associated volume. Any uncommitted or unpushed work
  in the bound volume is lost.
</Callout>
...
<Callout type="info">
  `zaru.docs` is handled entirely by the `zaru-mcp-server`. It searches the published documentation
