...
description: Complete input and output schemas for all 64 AEGIS MCP tools — 61 built-in orchestrator tools and the Zaru-only zaru.init, zaru.mode, and zaru.docs tools — with availability by context and Zaru conversation mode.
---
...
Tool count: **64** total — 61 built-in orchestrator tools + 3 Zaru client tools (`zaru.init`, `zaru.mode`, `zaru.docs`).
...
`aegis.schema.get`, `aegis.schema.validate`, `aegis.runtime.list`, `zaru.init`, `zaru.mode`, `zaru.docs`
...
| Zaru Chat — operator mode | All 64 tools | No filtering applied |
...
All 64 tools are available — no filtering is applied.
