...
**Requires configuration:** An API key must be set via the `api_key` field on the `web.search` entry in `spec.builtin_dispatchers`. See [Configuring Tools — Enabling `web.search`](/docs/guides/configuring-tools#part-b-enabling-websearch) for setup instructions.
