...
<Callout type="info">
  For complete input and output schemas for all built-in tools, see the [MCP
  Tool Reference](/docs/reference/mcp-tool-reference). This page documents the
  `skip_judge` policy, argument validation, and the per-tool compiled-in
  defaults.
</Callout>
...
<Callout type="info">
  `skip_judge` applies **only to agents executing inside the 100monkeys inner
  loop** (i.e. calls routed through `bootstrap.py` via the Dispatch Protocol).
  External agents calling `/v1/seal/invoke` directly are subject to SEAL
  authentication, policy, and argument validation (see [Argument
  Validation](#argument-validation) below) but are **never** sent to the
  semantic judge.
</Callout>
...
The canonical explanation of this control is in [Tool-Call Judging](/docs/agents/tool-call-judging).
...
| `aegis.execution.file`           | `true`               | Read-only — retrieves a file from a completed execution's workspace volume. Text files include inline content; binary files (images, PDFs, etc.) include a download URL for out-of-band retrieval.                                    |
...
| `aegis.file.list`                | `true`               | Read-only volume directory listing                                                                                                                                                                                                    |
| `aegis.file.read`                | `true`               | Read-only file content read                                                                                                                                                                                                           |
| `aegis.file.write`               | `false`              | State-mutating — writes file content in a user volume                                                                                                                                                                                 |
| `aegis.file.delete`              | `false`              | Destructive — removes a file or directory from a user volume                                                                                                                                                                          |
| `aegis.file.mkdir`               | `false`              | State-mutating — creates a directory in a user volume                                                                                                                                                                                 |
| `aegis.volume.create`            | `false`              | State-mutating — provisions a new persistent volume                                                                                                                                                                                   |
| `aegis.volume.list`              | `true`               | Read-only volume listing                                                                                                                                                                                                              |
| `aegis.volume.delete`            | `false`              | Destructive — deletes a volume and all contents                                                                                                                                                                                       |
| `aegis.volume.quota`             | `true`               | Read-only quota usage lookup                                                                                                                                                                                                          |
| `aegis.git.clone`                | `false`              | State-mutating — creates a binding and triggers clone                                                                                                                                                                                 |
| `aegis.git.list`                 | `true`               | Read-only binding listing                                                                                                                                                                                                             |
| `aegis.git.status`               | `true`               | Read-only clone/refresh status lookup                                                                                                                                                                                                 |
| `aegis.git.refresh`              | `false`              | State-mutating — fetches and checks out the pinned ref                                                                                                                                                                                |
| `aegis.git.delete`               | `false`              | Destructive — soft-deletes binding and cascades to volume                                                                                                                                                                             |
| `aegis.git.commit`               | `false`              | State-mutating — stages and commits workdir changes                                                                                                                                                                                   |
| `aegis.git.push`                 | `false`              | State-mutating — pushes commits to the remote                                                                                                                                                                                         |
| `aegis.git.diff`                 | `true`               | Read-only unified diff of workdir or index                                                                                                                                                                                            |
| `aegis.script.save`              | `false`              | State-mutating — persists a new TypeScript program                                                                                                                                                                                    |
| `aegis.script.list`              | `true`               | Read-only script listing                                                                                                                                                                                                              |
| `aegis.script.get`               | `true`               | Read-only script fetch with version history                                                                                                                                                                                           |
| `aegis.script.update`            | `false`              | State-mutating — updates a script and bumps its version                                                                                                                                                                               |
| `aegis.script.delete`            | `false`              | Destructive — soft-deletes a saved script                                                                                                                                                                                             |
...
| `aegis.execution.file`                                                                               | `execution_id`, `path`                          |
...
| `web.search`                                                                                         | `query`                                         |
| `web.fetch`                                                                                          | `url`                                           |
| `aegis.file.list`, `aegis.file.read`, `aegis.file.delete`, `aegis.file.mkdir`                        | `volume_id`, `path`                             |
| `aegis.file.write`                                                                                   | `volume_id`, `path`, `content`                  |
| `aegis.volume.create`                                                                                | `label`, `size_limit_bytes`                     |
| `aegis.volume.delete`                                                                                | `volume_id`                                     |
| `aegis.git.clone`                                                                                    | `repo_url`, `label`                             |
| `aegis.git.status`, `aegis.git.refresh`, `aegis.git.delete`, `aegis.git.push`, `aegis.git.diff`      | `binding_id`                                    |
| `aegis.git.commit`                                                                                   | `binding_id`, `message`                         |
| `aegis.script.save`                                                                                  | `name`, `code`                                  |
| `aegis.script.get`, `aegis.script.delete`                                                            | `id`                                            |
| `aegis.script.update`                                                                                | `id`, `name`, `code`                            |

Tools with no required parameters (e.g. `aegis.agent.list`, `aegis.tools.list`, `aegis.system.info`, `aegis.runtime.list`, `aegis.volume.list`, `aegis.volume.quota`, `aegis.git.list`, `aegis.script.list`) accept any object including `{}`. `aegis.runtime.list` accepts an optional `language` string to filter results by language name (e.g. `"python"`, `"go"`). `aegis.tools.search` accepts an optional `query` string but has no required fields. `aegis.agent.search` and `aegis.workflow.search` require a `query` string. `aegis.git.list` accepts optional `page` / `limit` integers. `aegis.script.list` accepts optional `tag` and `q` filter strings.
...
The `web.*` tools provide native HTTP request and web search capabilities.

### `web.search`

Performs an internet search query using the [Brave Search REST API](https://brave.com/search/api/). This is a native builtin — no external MCP server process is required.

**Requires configuration:** An API key must be set via the `api_key` field on the `web.search` entry in `spec.builtin_dispatchers`. See [Configuring Tools — Enabling `web.search`](/docs/agents/configuring-tools#part-b-enabling-websearch) for setup instructions.
...
- Configuration error if `api_key` is not set: `web.search is not configured: add api_key to the web.search entry in spec.builtin_dispatchers`
- `DomainNotAllowed` if the SecurityContext's `domain_allowlist` does not permit the Brave Search API domain
- Network error if the external request fails
...
Three system-tier tools handle multi-host fleet operations against user-installed edge daemons. They are gated by an operator/tenant-admin SecurityContext and surface in operator-mode conversations and the Zaru Fleet Launcher.

### `aegis.edge.fleet.invoke`

Invoke any registered tool against an `EdgeTarget` (`Node` / `Group` / `Selector` / `All`) with a `FleetDispatchPolicy`. Per-node `CommandProgress` chunks stream back tagged with `node_id`; the terminal payload is a `FleetExecutionResult` carrying `targets_resolved`, `targets_skipped`, `per_node`, and a summary.

The dispatch policy controls fan-out:

- **Mode**: `Sequential`, `Parallel`, or `Rolling { batch }`.
- **Failure policy**: `FailFast`, `ContinueOnError`, or `StopAfter(N)`.
- **`require_min_targets`**: refuses upfront if fewer matches resolve.
- **`per_target_deadline_secs`**: propagated to each daemon as `EdgeCommand.deadline`.

Tool descriptors carry a `fleet_capable: bool` flag — tools that are inherently single-target refuse multi-target dispatch even when the caller tries to fan them out.

### `aegis.edge.fleet.list`

Resolve an `EdgeTarget` to a node list **without** dispatching. Operationally critical before destructive fan-outs — Zaru's selector preview panel is built on this tool.

### `aegis.edge.fleet.cancel`

Broadcast `Cancel { command_id }` to every in-flight per-node command in a fleet operation, addressed by `fleet_command_id`. Already-completed nodes are unaffected.

For the full fleet model, see [fleet operations](/docs/concepts/fleet-operations); for the REST shape, see [edge REST API](/docs/reference/edge-rest-api).
