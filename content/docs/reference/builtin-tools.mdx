...
Tools with no required parameters (e.g. `aegis.agent.list`, `aegis.tools.list`, `aegis.system.info`) accept any object including `{}`. `aegis.tools.search` accepts an optional `query` string but has no required fields.
