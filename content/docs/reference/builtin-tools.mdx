...
| `aegis.workflow.cancel` | `false` | State-mutating — terminates a running workflow execution |
| `aegis.workflow.signal` | `false` | State-mutating — delivers human input to a paused workflow |
| `aegis.workflow.remove` | `false` | Destructive — deletes a workflow execution record |
...
| `aegis.task.status`, `aegis.task.wait`, `aegis.task.cancel`, `aegis.task.remove` | `execution_id` |
| `aegis.workflow.cancel`, `aegis.workflow.remove` | `execution_id` |
...
Tools with no required parameters (e.g. `aegis.agent.list`, `aegis.tools.list`, `aegis.system.info`) accept any object including `{}`. `aegis.tools.search` accepts an optional `query` string but has no required fields. `aegis.agent.search` and `aegis.workflow.search` require a `query` string.
