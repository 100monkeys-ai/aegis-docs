...
| `price_ids.pro_monthly`        | string | No       | —       | Stripe Price ID for Pro tier, monthly billing.                                                                      |
| `price_ids.pro_annual`         | string | No       | —       | Stripe Price ID for Pro tier, annual billing.                                                                       |
| `price_ids.business_monthly`   | string | No       | —       | Stripe Price ID for Business tier, monthly billing.                                                                 |
| `price_ids.business_annual`    | string | No       | —       | Stripe Price ID for Business tier, annual billing.                                                                  |
| `price_ids.enterprise_monthly` | string | No       | —       | Stripe Price ID for Enterprise tier, monthly billing.                                                               |
| `price_ids.enterprise_annual`  | string | No       | —       | Stripe Price ID for Enterprise tier, annual billing.                                                                |

All fields support `env:` and `secret:` credential prefixes (see [Credential Resolution](#credential-resolution)).

> **Note:** `publishable_key` and `pricing_table_id` are frontend-only variables configured on the Zaru client (via `STRIPE_PUBLISHABLE_KEY` and `STRIPE_PRICING_TABLE_ID` environment variables), not in the orchestrator config.
