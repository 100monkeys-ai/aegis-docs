...
spec:
  ...
  runtime:
    ...
    container_network_mode: "aegis-exec-network" # Isolated network for agent containers
  ...
  # ─── Network ────────────────────────────────────────────────────────────────
  network:
    bind_address: "127.0.0.1" # Default. Bind a non-loopback address only with TLS
    # configured below — or, when TLS is terminated upstream by a trusted-network
    # ingress (e.g. Caddy in the same pod-network), with `allow_insecure_bind: true`.
    port: 8088
    grpc_port: 50051
    ...
    allow_insecure_bind: false # Operator-acknowledged opt-out — see field reference below
    ...
    - name: "fs"
      ...
      capabilities:
        ...
        - name: fs.delete
          skip_judge: false # state-mutating — always validate
    - name: "web.search"
      description: "Native web search powered by the Brave Search API"
      enabled: true
      api_key: "env:BRAVE_SEARCH_API_KEY" # obtain a free key at https://brave.com/search/api/
      capabilities:
        - name: web.search
  ...
  observability:
    ...
    metrics:
      ...
      # Defaults to 127.0.0.1 (loopback). Off-host scraping requires
      # an explicit non-loopback value AND an authenticating reverse proxy
      # in front of the metrics port — never expose this directly.
...
| _(bare string)_ | `sk-abc123...`                   | Plaintext. Avoid for secrets.                                      |
...
| `models[].max_output_tokens`  | u32      | ❌       | `8192`  | Max output tokens per request. Increase for thinking-mode models.                                                                                     |
| `models[].temperature`        | f32      | ❌       | `0.4`   | Default LLM sampling temperature for this alias. `0.0` = deterministic, `1.0` = creative. Can be overridden per-agent via `spec.runtime.temperature`. |
...
| `container_socket_path`  | string | _(platform default)_    | Container runtime socket path. The orchestrator auto-detects whether the socket belongs to Docker or Podman. Common values: `/var/run/docker.sock` (Docker), `/run/user/<UID>/podman/podman.sock` (Podman rootless), `/run/podman/podman.sock` (Podman rootful). If omitted, the orchestrator checks `CONTAINER_HOST`, then `DOCKER_HOST`, then falls back to the platform default (`/var/run/docker.sock`). |
...
| Key                          | Type   | Default     | Description                                                                                                                                                                  |
| ---------------------------- | ------ | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bind_address`               | string | `127.0.0.1` | Network interface to bind all listeners. Defaults to loopback; binding a non-loopback address requires either `tls` configured or `allow_insecure_bind: true`.               |
...
| `allow_insecure_bind`        | bool   | `false`     | Operator-acknowledged opt-out of the non-loopback-without-TLS startup gate. Set `true` ONLY when TLS is terminated upstream by a trusted-network ingress. See callout below. |

<Callout type="warn">
**`allow_insecure_bind` is an opt-out, not a default.** AEGIS refuses to start when `bind_address` is a non-loopback address (e.g. `0.0.0.0`) and `tls` is not configured — every public request would otherwise transit cleartext.

Set `allow_insecure_bind: true` only when **all** of the following are true:

- TLS is terminated **upstream** of this pod by a trusted-network ingress (Caddy, nginx, or an equivalent reverse proxy in the same Podman pod-network).
- The pod's bind endpoint is reachable **only** from inside that trusted network — never from the public internet.
- You accept the audit-trail consequence: a `WARN` entry is emitted on every boot recording that the override is engaged.

Setting `allow_insecure_bind: true` on a publicly-exposed host is a serious security regression — every external request would transit cleartext.
...
| Key            | Type               | Default | Description                                                                                                                                                     |
| -------------- | ------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`         | string             | —       | Dispatcher identifier (e.g. `"cmd"`, `"fs"`, `"web.search"`)                                                                                                    |
...
| `api_key`      | string             | null    | API key for builtins that call external services. Supports `env:VAR_NAME` and `secret:path/to/secret` resolution. Required for `web.search` (Brave Search API). |
...
Optional. Configures Stripe billing integration. Omit this section for self-hosted deployments where billing is not required. When `stripe_secret_key` is absent, all `/v1/billing/*` endpoints return `501 Not Implemented`.

| Key                     | Type   | Required | Default | Description                                                                                                         |
| ----------------------- | ------ | -------- | ------- | ------------------------------------------------------------------------------------------------------------------- |
| `stripe_secret_key`     | string | Yes      | —       | Stripe API secret key (server-side only). Supports `env:` and `secret:` prefixes.                                   |
| `stripe_webhook_secret` | string | No       | —       | Webhook signing secret for verifying Stripe event signatures (HMAC-SHA256). Supports `env:` and `secret:` prefixes. |

All fields support `env:` and `secret:` credential prefixes (see [Credential Resolution](#credential-resolution)).

Pricing is dynamic — the orchestrator fetches available tiers and prices directly from Stripe at runtime via `GET /v1/billing/prices`. No price IDs are configured in NodeConfig.

> **Note:** `STRIPE_PUBLISHABLE_KEY` is a frontend-only environment variable configured on the Zaru client container, not in the orchestrator config.
...
| `role`                      | enum     | ✅       | `hybrid` | Node role: `controller` \| `worker` \| `hybrid` \| `edge` \| `relay-coordinator` |
...
The five roles below are the **platform topology** primitives for self-hosted AEGIS clusters. `controller`, `worker`, and `hybrid` are the classic multi-node primitives; `edge` and `relay-coordinator` extend the model to user-host daemons and to the SaaS multi-tenant relay process. The Zaru SaaS deployment runs `relay.myzaru.com` as a `relay-coordinator` operated by 100monkeys; self-hosters can deploy their own.
...
| `edge`              | Outbound only — daemon dials its controller                   | Controller (or `relay-coordinator`) advertised in the enrollment token's `cep` claim                   |
| `relay-coordinator` | `NodeClusterService` on `cluster_grpc_port` (default 50056)   | Backed by Postgres / IAM / OpenBao; brokers gRPC streams from `edge` nodes for the controller's tenant |
...
| `metrics.bind_address` | string | `127.0.0.1` | Network interface the Prometheus exporter binds. Defaults to loopback to prevent unauthenticated network exposure of internal counters and labels. Set a non-loopback address only when the endpoint is fronted by an authenticating reverse proxy. |
...
spec:
  ...
  runtime:
    ...
    container_network_mode: "env:AEGIS_CONTAINER_NETWORK" # Should resolve to "aegis-exec-network"
    orchestrator_url: "env:AEGIS_ORCHESTRATOR_URL"
    nfs_server_host: "env:AEGIS_NFS_HOST"
...
## Edge Mode Roles and `spec.edge`

This section is the **platform reference** for the `edge` and `relay-coordinator` cluster roles — the values you set when self-hosting AEGIS and configuring node topology directly. If you are a Zaru SaaS user installing `aegis edge` on your laptop or server, the daemon's `cluster.role` is set automatically by `aegis edge enroll` and the SaaS Relay Coordinator at `relay.myzaru.com` is operated by 100monkeys — see [Edge Host Setup](/docs/zaru/edge-host-setup) for that flow.

Two values join the `cluster.role` enum to support edge topologies:

| Role                | Purpose                                                                                                                                         |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `edge`              | A user-installed daemon running on a host the user controls. Tenant-bound. Connects outbound to a controller (OSS) or Relay Coordinator (SaaS). |
| `relay-coordinator` | The SaaS multi-tenant relay process. Brokers connections from edge daemons. Re-uses the existing `NodeClusterService` code path.                |

Existing roles (`controller`, `worker`, `hybrid`) accept edge enrollments alongside their existing duties when configured to do so; `relay-coordinator` is the dedicated process for SaaS.

### `spec.edge` (required when `cluster.role: edge`)
...
cluster:
  ...
  edge:
    ...
    capabilities:
      ...
      custom_labels: { region: home }
```

### `cluster.ingress` (required when `cluster.role: relay-coordinator`)
...
| `role: edge` and `cluster.edge.tenant_id` missing                       | Reject — the daemon is unenrolled.          |
| `role: relay-coordinator` and `cluster.ingress.public_endpoint` missing | Reject.                                     |

For the full edge config field reference, see [edge config](/docs/reference/edge-config).
