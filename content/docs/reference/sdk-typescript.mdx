...
| | `executeAgent(agentId, input, intent?, contextOverrides?, attachments?)` | Execute an agent directly (alias route) |
...
| | `runWorkflow(name, payload)` | Run a workflow by name (payload may include `attachments`) |
| | `executeWorkflow(name, input, attachments?)` | Run a workflow with typed input and optional attachments |
...
| **Execution** | | |
| | `startExecution(agentId, input, contextOverrides?, intent?, attachments?)` | Start an agent execution |
| | `attachToVolume(volumeId, blobOrPath)` | Stream a file into a user volume; returns an `AttachmentRef` |
...
### `executeAgent(agentId, input, intent?, contextOverrides?, attachments?)`
...
| `attachments` | `AttachmentRef[]` | Optional file references the agent should be able to read. Each entry has `volumeId`, `path`, `name`, `mimeType`, `size`, and optional `sha256`. Build entries with `attachToVolume`. |
...
### `startExecution(agentId, input, contextOverrides?, intent?, attachments?)`
...
| `attachments` | `AttachmentRef[]` | Optional file references. The receiving agent must declare `attachments` in `spec.input_schema` and include `aegis.attachment.read` in `spec.tools`. |
...
### `attachToVolume(volumeId, blobOrPath)`

Stream a file into a user volume and return an `AttachmentRef` ready to pass to `startExecution`,
`executeAgent`, `executeWorkflow`, or any other dispatch entrypoint that accepts an `attachments`
array. Pass the reserved name `"chat-attachments"` as `volumeId` to lazy-provision the standard
chat-uploads volume on first call.

```typescript
const ref = await client.attachToVolume('chat-attachments', file);
const { execution_id } = await client.executeAgent(
  'document-summarizer',
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `volumeId` | `string` | UUID of an existing user volume, or the literal `"chat-attachments"` to lazy-provision the chat-uploads volume on first call. |
| `blobOrPath` | `Blob \| File \| string` | A `Blob`/`File` (browser) or a filesystem path (Node). The upload is streamed — no in-memory buffering. |

**Returns:** `Promise<AttachmentRef>` with `volumeId`, `path`, `name`, `mimeType`, `size`, and `sha256`.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `options.price_id` | `string` | Stripe Price ID for the base tier |
| `options.seat_price_id` | `string?` | Stripe Price ID for per-seat billing |
| `options.seats` | `number?` | Number of additional seats |
