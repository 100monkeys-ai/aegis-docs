...
The registry is stored as a single YAML document. The canonical full file lives in [aegis-orchestrator/runtime-registry.yaml](https://github.com/100monkeys-ai/aegis-orchestrator/blob/main/runtime-registry.yaml).
  ...
  runtimes:
    ...
    typescript:
      "5.1":
        ...
        description: "TypeScript 5.1 via Node.js 20 with Alpine base; global install required"
```
...
`aegis.runtime.list` is available in the `aegis-system-agent-runtime` security context with no additional configuration. See the [Management Tools reference](/docs/reference/management-tools#aegisruntimelist) for the full parameter and response schema.
