...
**Returns:** A paginated list of execution summaries including `execution_id`, `workflow_id`, `status`, `current_state`, `summary` (string or `null` — a derived, whitespace-collapsed preview of the execution's `intent`, capped at 160 characters with `…` overflow), and timestamps.
...
**Returns:** A list of execution summaries including `id`, `agent_id`, `status`, `started_at`, and `summary` (string or `null` — a derived, whitespace-collapsed preview of the execution's `intent`, capped at 160 characters with `…` overflow).
