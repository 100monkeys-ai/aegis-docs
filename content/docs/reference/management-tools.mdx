...
| `poll_interval_seconds` | integer | No | Seconds between internal polls (default: 10) |
...
Returns all supported standard runtime environments from the `StandardRuntimeRegistry`. Use this tool before setting `spec.runtime` in an agent manifest to ensure the target `language` and `version` combination is valid and not deprecated. Available in the `agent-runtime` security context — no additional configuration required.
