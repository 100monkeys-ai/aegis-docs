...
| | `execute_agent(agent_id, input, intent?, context_overrides?, attachments?)` | Execute an agent directly (alias route) |
...
| | `run_workflow(name, payload)` | Run a workflow by name (payload may include `attachments`) |
| | `execute_workflow(name, input, attachments?)` | Run a workflow by name with typed input and optional attachments |
...
| **Execution** | | |
| | `start_execution(agent_id, input, intent?, context_overrides?, attachments?)` | Start an agent execution |
| | `attach_to_volume(volume_id, file)` | Stream a local file into a user volume; returns an `AttachmentRef` |
...
#### `execute_agent(agent_id, input, intent?, context_overrides?, attachments?)`
...
| `attachments` | `Optional[List[AttachmentRef]]` | File references to forward to the agent. Each entry has `volume_id`, `path`, `name`, `mime_type`, `size`, and an optional `sha256`. Build entries with `attach_to_volume`. |
...
#### `start_execution(agent_id, input, intent?, context_overrides?, attachments?)`
...
| `context_overrides` | `Any` | Optional context overrides |
| `attachments` | `Optional[List[AttachmentRef]]` | File references the agent should be able to read. Each entry has `volume_id`, `path`, `name`, `mime_type`, `size`, and optional `sha256`. The receiving agent must declare `attachments` in `spec.input_schema` and include `aegis.attachment.read` in `spec.tools`. |

**Returns:** `StartExecutionResponse` with `execution_id: str`.

---

#### `attach_to_volume(volume_id, file)`

Stream a local file into a user volume and return an `AttachmentRef` ready to pass to
`start_execution`, `execute_agent`, `execute_workflow`, or any other dispatch entrypoint that
accepts an `attachments` array. Pass the reserved name `"chat-attachments"` as `volume_id` to
lazy-provision the standard chat-uploads volume on first call.

```python
async with AegisClient(...) as client:
    ref = await client.attach_to_volume("chat-attachments", "report.pdf")
    result = await client.execute_agent(
        agent_id="document-summarizer",
        input="Produce a one-paragraph executive summary.",
        attachments=[ref],
    )
```
...
| Name | Type | Description |
|------|------|-------------|
| `volume_id` | `str` | UUID of an existing user volume, or the literal `"chat-attachments"` to lazy-provision the chat-uploads volume on first call. |
| `file` | `str \| Path \| BinaryIO` | Path to a local file or an open binary file-like object. The upload is streamed — bytes are not buffered in memory. |

**Returns:** `AttachmentRef` with `volume_id`, `path`, `name`, `mime_type`, `size`, and `sha256`.
...
- [Custom Runtime Agents](/docs/agents/custom-runtime-agents) — using a custom `bootstrap.py`
