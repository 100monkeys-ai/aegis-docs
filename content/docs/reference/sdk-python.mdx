...
- **`AegisClient`** — async HTTP client: start executions, manage approvals, invoke SMCP tools, administer tenants and rate limits
...
| SMCP | `attest_smcp(payload)` | Obtain an SMCP security attestation token |
| SMCP | `invoke_smcp(payload)` | Invoke an SMCP tool |
| SMCP | `list_smcp_tools(security_context?)` | List available SMCP tools |
...
Obtain a security attestation token for SMCP tool invocation.

**Returns:** `SmcpAttestationResponse` with `security_token: str`.

---

#### `invoke_smcp(payload)`

Invoke an SMCP tool with an attested payload.

**Returns:** `Dict[str, Any]` — tool-specific response.

---

#### `list_smcp_tools(security_context?)`

List available SMCP tools, optionally filtered by security context.

**Returns:** `SmcpToolsResponse` with `protocol`, `attestation_endpoint`, `invoke_endpoint`, `security_context`, and `tools: List[Any]`.
