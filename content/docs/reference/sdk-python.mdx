...
| **Billing** | | |
| | `create_checkout_session(tier, interval)` | Create a Stripe Checkout Session |
...
#### `create_checkout_session(tier, interval)`

Create a Stripe Checkout Session URL for subscribing to a plan.

```python
resp = await client.create_checkout_session("pro", "monthly")
...
| Name | Type | Description |
|------|------|-------------|
| `tier` | `str` | Subscription tier: `"pro"`, `"business"`, or `"enterprise"` |
| `interval` | `str` | Billing interval: `"monthly"` or `"annual"` |
