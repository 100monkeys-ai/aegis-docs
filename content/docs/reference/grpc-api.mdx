...
const client = new proto.AegisRuntime(
  "localhost:50051",
...
stream.on("data", (event: any) => {
  if (event.execution_started) {
    ...
    console.log(
      `Iteration ${event.iteration_started.iteration_number}: ${event.iteration_started.action}`,
    );
...
Issue an SEAL SecurityToken to an agent container after verifying its identity. This is the first call an agent makes before any tool invocations are possible. See the [SEAL reference](/docs/gateway/seal-protocol) for the full attestation flow.
...
The `WorkflowExecution` message returned by workflow-related RPCs includes output schema fields:
...
message WorkflowExecution {
  ...
  string blackboard_json           = 5;  // JSON-encoded final Blackboard
  optional string final_output_json = 6; // JSON-encoded final_output (when output_template is declared)
  string started_at                = 7;
  optional string completed_at     = 8;
}
```

The `final_output_json` field contains the structured output produced by evaluating the workflow's `output_template` against the final Blackboard. It is `null` when the workflow does not declare an output template or has not yet completed.
...
**Port:** FUSE daemon listen address (default `127.0.0.1:50053`)
...
## Edge Mode Extensions to `NodeClusterService`

Two new RPCs extend the existing `NodeClusterService` for edge daemon support. There is no separate `EdgeService` — daemon attestation, registration, rotation, and stream all flow through the same service on port `50056`.
...
A bidirectional stream. The daemon sends `EdgeEvent` (Hello, Heartbeat, CommandResult, CommandProgress, CapabilityUpdate); the server sends `EdgeCommand` (InvokeTool, Cancel, PushConfig, Drain, Shutdown). Every message carries a `SealNodeEnvelope` proving daemon identity. `InvokeTool` carries an inner `SealEnvelope` proving user identity — the **two-envelope SEAL pattern**.
...
Atomic key rotation with a **dual-signature** requirement: the request is signed by both the old key (proof of authority) and the new key (proof of possession). The server commits the rotation in a single PostgreSQL transaction (update pubkey, blacklist old token, issue new token) and records both pubkeys for an overlap window so the active gRPC stream survives the swap.

### `ChallengeNode` bootstrap proof
...
Set when `role = NODE_ROLE_EDGE`. The server validates the JWT, atomically redeems `jti`, persists the `EdgeDaemon` row, and issues a `NodeSecurityToken` with a new `tid` claim — the persistent tenant binding for the daemon's lifetime.

For the full edge gRPC reference (message shapes, error codes, transport details), see [relay gRPC API](/docs/reference/relay-grpc-api).
