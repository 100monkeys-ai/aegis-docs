...
Issue an SMCP SecurityToken to an agent container after verifying its identity. This is the first call an agent makes before any tool invocations are possible. See the [SMCP reference](/docs/architecture/smcp) for the full attestation flow.
...
- Also available over REST at `POST /v1/smcp/attest`.

---

## `InvokeTool`

Invoke a tool on behalf of an agent via orchestrator mediation. Agents do not call MCP servers directly — all tool calls are routed through the orchestrator, which verifies the SMCP envelope before forwarding. See [Tool Routing](/docs/architecture/tool-routing) for the three routing paths.

```protobuf
rpc InvokeTool(InvokeToolRequest) returns (InvokeToolResponse);

message InvokeToolRequest {
  string security_token = 1; // JWT from AttestAgentResponse
  string signature      = 2; // Ed25519 signature of inner_mcp bytes (base64)
  bytes  inner_mcp      = 3; // JSON-RPC payload for the MCP tool call
}
...
- `inner_mcp` is a standard MCP JSON-RPC `call_tool` request serialized to bytes.
- `signature` must be the Ed25519 signature over `inner_mcp`, produced with the ephemeral private key registered during `AttestAgent`.
...
- Also available over REST at `POST /v1/smcp/invoke`.
...
Ingest an external stimulus and route it to a workflow via the two-stage hybrid pipeline (ADR-021).
...
All RPCs (except `AttestNode`) require a valid `NodeSecurityToken` in the `authorization` metadata key and an `SmcpNodeEnvelope` wrapping the payload. `AttestNode` is unauthenticated (it is the first call a new worker makes).
...
The `NodeSecurityToken` is an RS256 JWT (1-hour TTL) signed by the controller's OpenBao Transit key. It is analogous to the agent `SecurityToken` from SMCP but scoped to node identity. All subsequent RPCs wrap their payload in:

```protobuf
message SmcpNodeEnvelope {
  string node_security_token = 1; // NodeSecurityToken JWT
  bytes  signature           = 2; // Ed25519 signature over inner_payload
  bytes  inner_payload       = 3; // Serialized request message
}
```
