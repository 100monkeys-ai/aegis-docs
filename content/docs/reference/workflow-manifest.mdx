...
If coercion or schema validation fails, the workflow completes but `final_output` is not populated. The failure is recorded in the workflow execution event log.
...
> **`output_template` requires `output_schema`.** The template defines how values are extracted; the schema defines their types and validation constraints. If `output_template` is present without `output_schema`, the manifest is rejected at deploy time.
