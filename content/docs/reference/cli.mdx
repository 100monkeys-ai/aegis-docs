...
Supported structured-output commands include agent, task, workflow, node, config, daemon, status, and update flows that return a bounded summary. Streaming commands such as `agent logs`, `task logs`, and `workflow logs` remain stream-oriented and do not use the generic `--output` renderer. The MCP counterpart for `aegis task logs` is `aegis.task.logs`, which returns paginated execution events instead of a live stream.
