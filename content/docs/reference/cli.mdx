...
aegis fuse-daemon start --listen-addr 127.0.0.1:50053
```
...
| `--listen-addr`      | `127.0.0.1:50053`        | Address the FUSE daemon listens on for mount/unmount RPCs from the orchestrator. |
...
aegis task execute pdf-summarizer --attachment 1bcd...:reports/q3.pdf --attachment 1bcd...:reports/q4.pdf
aegis task execute pdf-summarizer --attachments @refs.json
```
...
| `--attachments <json>`          | none         | File references in the canonical JSON form. Inline JSON array of `AttachmentRef` objects (`[{"volume_id":"...","path":"...","name":"...","mime_type":"...","size":N,"sha256":"..."}]`) or `@file` to read from disk. The receiving agent must declare `attachments` in `spec.input_schema` and include `aegis.attachment.read` in `spec.tools`. |
| `--attachment <volume_id:path>` | none         | Shorthand. Repeatable. Each occurrence names one file by `volume_id` and `path` within the volume; the CLI stat-s the file via the orchestrator's volume metadata API and constructs the corresponding `AttachmentRef` (filling in `name`, `mime_type`, `size`, and `sha256`). Mutually exclusive with `--attachments`.                         |
...
Use `--attachment` for ad-hoc invocations where you know the volume and path; use `--attachments` (or `--attachments @file.json`) when the references come from automation, an API response, or a script that has already gathered the full `AttachmentRef` shape. Passing both forms in one invocation is rejected before dispatch.
...
aegis workflow run review-pipeline --attachment 1bcd...:reports/q3.pdf
aegis workflow run review-pipeline --attachments @refs.json
```
...
| `--attachments <json>`          | none         | File references in the canonical JSON form. Inline JSON array of `AttachmentRef` objects or `@file`. Forwarded to every agent state that declares `attachments` in its `input_schema`.                              |
| `--attachment <volume_id:path>` | none         | Shorthand. Repeatable. Each occurrence names one file by `volume_id` and `path`; the CLI resolves it to a full `AttachmentRef` via the orchestrator's volume metadata API. Mutually exclusive with `--attachments`. |
...
The `aegis edge` family is an exception — it publishes a stable exit-code contract documented under [edge CLI reference](/docs/reference/edge-cli#exit-codes).

---

## Node Commands — Two Subsections

The `aegis` CLI's node-related commands split into two distinct categories. Both operate on `NodeClusterService` but address different topologies.

### Orchestrator Cluster (`aegis cluster`, `aegis node`)

Commands for managing the **internal cluster** — controllers, workers, hybrid nodes that form the AEGIS execution plane. These are tenant-agnostic infrastructure nodes.

- `aegis cluster nodes` — list cluster members.
- `aegis cluster push-config <node-id>` — push a hierarchical config delta.
- `aegis node attest` — re-attest a worker.
- `aegis node forward-execution` — route an execution to a specific worker.

### Edge Daemon (`aegis edge`)

Commands for **user-installed daemons** running on hosts the user controls. Edge daemons are tenant-bound, dispatched-to (not orchestrator peers), and address a different operational shape.

- `aegis edge enroll <token>` — daemon-side: redeem an enrollment token, persist node identity.
- `aegis edge status` — daemon-side: show local connection state.
- `aegis edge ls` — operator-side: list edges in your tenant.
- `aegis edge tag` / `aegis edge group` — manage tags and groups.
- `aegis edge fleet preview|run|cancel|runs` — fleet operations.
- `aegis edge keys rotate|revoke-remote` / `aegis edge token refresh` — identity hygiene.

For the full surface, see [edge CLI reference](/docs/reference/edge-cli).
