...
<Callout type="warn">
  The OpenBao service shown above runs in dev mode with an in-memory backend and
  an unsealed root token. For production, run a real OpenBao cluster with
...
For the user-facing edge-tools surface, see [edge mode overview](/docs/concepts/edge-mode-overview); for the security model, see [edge security](/docs/concepts/edge-security).
