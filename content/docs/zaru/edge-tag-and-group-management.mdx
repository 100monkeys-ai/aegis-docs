...
<Callout type="info">
  **Self-hosting AEGIS instead?** The **Vault → Edge Hosts → Groups** UI
  described here is the Zaru SaaS surface. The `aegis edge tag` and `aegis edge
  group` CLI commands work identically against a self-hosted controller — see
  [Edge CLI Reference](/docs/reference/edge-cli) and [Edge Mode
  Overview](/docs/concepts/edge-mode-overview).
</Callout>
