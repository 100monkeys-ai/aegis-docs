...
```
Root Execution (ZaruSession / parent)
...
This prevents privilege escalation via spawned children. A `zaru-free` tier parent cannot spawn a child with `zaru-pro` capabilities.
