...
- **Zaru Conversation Modes**: Conversations have four user-selected modes — `ZaruConversationMode::Chat`, `ZaruConversationMode::Agentic`, `ZaruConversationMode::Architect`, and `ZaruConversationMode::Operator` — chosen at conversation creation and locked once the first message is sent. For example, `ZaruConversationMode::Architect` transitions the interface into a specialized state for interactively defining AEGIS workflow manifests.
