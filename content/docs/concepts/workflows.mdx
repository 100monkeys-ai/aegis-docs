...
| Feature | Temporal Workflows (`Workflow`) | SMCP ToolWorkflows (`Macro-Tools`) |
|---|---|---|
| **Purpose** | Async CI/CD pipelines, long-running multi-agent choreography. | Sync REST API macro-tools, context window compression. |
| **Backend** | Temporal (Durable, stateful, persistent). | SMCP Tooling Gateway (Stateless, sub-second). |
...
This page documents **Temporal Workflows**. For documentation on REST macro-tools, see the [SMCP Tooling Gateway](/docs/architecture/tooling-gateway) architecture.
