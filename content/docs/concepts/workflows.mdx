...
This page documents **Temporal Workflows**. For documentation on REST macro-tools, see the [SEAL Tooling Gateway](/docs/architecture/tooling-gateway) architecture.
...
aegis workflow list --visible
```

Direct jumps between User and Global are not allowed — you must promote through Tenant first to enforce a review step.
...
A workflow manifest declares the FSM states and transitions, along with optional metadata including `input_schema` (what the workflow accepts), `output_schema` (the shape of the workflow's structured output), and `output_template` (Handlebars expressions that extract the final output from the Blackboard at completion).
...
To resume a suspended workflow, send a signal through the workflow execution API:
...
1. **Seed** — Before the workflow starts, the orchestrator writes all `spec.context` keys into the Blackboard and then merges any startup `blackboard` overrides. Startup overrides must be a JSON or YAML object and win on same-named top-level keys. The reserved `workflow` key cannot be overridden.
2. **Live execution** — The Temporal worker owns the Blackboard during execution. It receives the merged startup Blackboard, preserves authoritative `workflow.*` metadata, and writes each completed state's output back under the state's name (for example `REQUIREMENTS.output`, `REQUIREMENTS.status`, `REQUIREMENTS.score`). All Handlebars templates in subsequent states are resolved against this live state.
...
aegis workflow run release-pipeline \
  --input @input.yaml \
  --blackboard @blackboard.yaml
```

Example `blackboard.yaml`:
...
- `--blackboard` accepts inline JSON/YAML or `@file` input.
- The value must be an object, not a scalar or array.
- Blackboard overrides are merged on top of `spec.context`.
- `workflow.*` metadata remains authoritative and cannot be replaced by overrides.
- The live Blackboard is passed into downstream Agent states as structured execution context.
...
command: "deploy --env {{blackboard.deploy_env}}"
```
...
Workflows can declare their output shape using `metadata.output_schema` and `metadata.output_template`. When the workflow reaches a terminal state, the engine evaluates each `output_template` value as a Handlebars expression against the final Blackboard, coerces the rendered values to the types declared in `output_schema`, and stores the result as `final_output` on the `WorkflowExecution`. This `final_output` is returned as `last_output` in `aegis.workflow.wait` tool responses, giving callers a structured, typed result without parsing raw agent text.

For full field definitions and type coercion rules, see the [Workflow Manifest Reference](/docs/reference/workflow-manifest#metadataoutput_schema).
...
aegis workflow run the-forge --input '{"task": "Build a REST API for user authentication", "repo_url": "https://github.com/org/repo"}'
```
...
Use `--force` when you are re-deploying the same workflow name and version.

For execution status and Human-state signaling, use the CLI:

```bash
aegis workflow executions get <execution-id>
aegis workflow signal <execution-id> --response approved
```

The underlying REST API remains available for integrations, but the CLI is the preferred operator path.

See the [Workflow Manifest Reference](/docs/reference/workflow-manifest) for the complete field specification.
See [CLI Capability Matrix](/docs/reference/cli-capability-matrix) for currently CLI-exposed workflow operations.
