...
The **Blackboard** is a mutable key-value store shared across all states in a workflow execution. It is initialised from `spec.context` (and the optional `blackboard_defaults` alias) and updated as states complete.
