...
While the outer-loop `validation` runs at the **end** of an iteration, AEGIS also supports **inner-loop** validation via the `tool_validation` field. For the full tool-call flow, judge payload contract, and operator bypass semantics, see [Tool-Call Judging](/docs/guides/tool-call-judging).
