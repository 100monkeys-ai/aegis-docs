...
| `task` | object | | Instruction and prompt template. Community skills can be imported via the [`skill-import` workflow](/docs/guides/importing-skills). |
...
| `volumes` | object[] | | Volume mounts. See [Configuring Storage](/docs/deployment/configuring-storage). |
...
See [Standard Runtime Registry](/docs/reference/standard-runtime-registry) for the full language-version table, and [Custom Runtime Agents](/docs/guides/custom-runtime-agents) for the custom image path.
...
See [Docker Deployment](/docs/deployment/docker), [Podman Deployment](/docs/deployment/podman), and [Firecracker Runtime](/docs/deployment/firecracker) for deployment details.
...
See [Configuring LLM Providers](/docs/deployment/llm-providers) for the full provider configuration reference.
