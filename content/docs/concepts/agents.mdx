...
The `AgentRuntime` trait abstracts both isolation backends. Switching a node from Docker to Firecracker requires only a config change — no agent manifest changes needed.
...
See [Docker Deployment](/docs/deployment/docker) and [Firecracker Runtime](/docs/deployment/firecracker) for deployment details.
