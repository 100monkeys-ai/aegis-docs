...
A direct route is a static mapping from a `source_name` to a `WorkflowId`. Register one via the CLI or management API:
...
```bash
echo '{"event": "deploy", "ref": "main"}' | aegis daemon
```
