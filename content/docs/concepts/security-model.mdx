...
AEGIS applies security contexts across two independent surfaces. The context in effect depends on _where_ the tool call is being made, not who triggered the chain.
...
- `aegis` read and execution tools (`aegis.agent.get`, `aegis.workflow.list`, `aegis.task.execute`, `aegis.execution.file`, etc.)
...
When a user installs an AEGIS edge daemon on their own host, the daemon must prove its identity to the controller (or Relay Coordinator) before any MCP tool call can be routed to it. The mechanism is built on the same SEAL primitives as the rest of the platform but adds two new patterns: a **one-time enrollment token** and a **two-envelope SEAL pattern** for command dispatch.

### One-time enrollment tokens

A user (or team admin) issues an enrollment token via Zaru. The token is a short-lived JWT (≤15 minutes) signed by the same OpenBao Transit key that signs `NodeSecurityToken`. Its claims include:

- `tid` — the tenant the daemon will be bound to.
- `cep` — the controller endpoint the daemon should connect to.
- `jti` — a one-time-use nonce, atomically redeemed via `INSERT … ON CONFLICT` against the `enrollment_tokens` table.

The user runs `aegis edge enroll <token>` on the host. The CLI generates an Ed25519 keypair, attests against the endpoint from the token, and on success receives a persistent `NodeSecurityToken` whose `tid` claim pins the daemon to its tenant for life. Replays are rejected at the database boundary; expiry rejects stale copies.

### Two-envelope SEAL pattern

Every command flowing from the platform to an edge daemon is wrapped in two SEAL envelopes:

- **Outer `SealNodeEnvelope`** — proves daemon identity at the node level. Signed by the daemon's Ed25519 node key.
- **Inner `SealEnvelope`** — proves user identity at the user level. Carries `user_security_token`, `tenant_id`, and `security_context_name`.

The daemon verifies both envelopes locally before executing. The same `security_context_name` resolved on the controller is resolved against the daemon's local merged config — Layer 1 infrastructure policy and Layer 2 SEAL discipline both apply on the edge, exactly as on the controller.

For the full edge security model — including key rotation, fleet-wide rotation, and revocation — see [edge security](/docs/concepts/edge-security).
