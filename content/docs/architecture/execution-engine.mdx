...
To start an LLM inference pass:
...
|---|---|---|
| `type` | yes | Always `"generate"` for an LLM inference request. |
...
| `model_alias` | no | LLM alias to route through. Defaults to `"default"` (orchestrator's configured default). Maps from `spec.runtime.model` in the agent manifest. |
| `messages` | no | Prior conversation turns to prepend. Empty array for a fresh iteration start. |

To return the result of a dispatched subprocess:
...
Example schema entry for `fs.write`:
...
```json
{
  ...
  "function": {
    "name": "fs.write",
    "description": "Write content to a file inside the agent's workspace volume.",
    "parameters": {
      "type": "object",
      "properties": {
...
Tool schemas surface through two layers. The node operator declares which tool servers exist in `aegis-config.yaml` (see [MCP Tool Routing](/docs/architecture/tool-routing)) — this is the ceiling of what any agent on that node can ever invoke. Each agent manifest then declares a `spec.tools` block that selects the subset of those node-level tools the agent is permitted to use and applies per-tool policy constraints (path allowlists, domain allowlists, `SubcommandAllowlist`, rate limits, and so on). Only the tools named in `spec.tools` are included in the schema array sent to the LLM; tools registered on the node but absent from the manifest are invisible to the agent. `bootstrap.py` itself never sees or manages tool schemas — schema injection and policy enforcement happen entirely in the orchestrator before the `generate()` response is returned.

When `execution.tool_validation` is configured for a tool, the orchestrator inserts one more gate before any routing path fires. The semantic judge runs as a child execution, compares the proposed tool intent against `min_score` and `min_confidence`, and either blocks the call or allows it to continue into the normal policy and routing pipeline. That checkpoint improves execution reliability; it does not replace SMCP authorization or tool policy enforcement.
...
Every `cmd.run` invocation is validated against the `SubcommandAllowlist` declared in the agent manifest's `spec.tools` block before execution. The node-level `builtin_dispatchers.cmd` section in `aegis-config.yaml` sets hard ceilings (max timeout, max output bytes, global env denylist) that the manifest cannot exceed, but the per-command allowlist itself lives in the manifest:
