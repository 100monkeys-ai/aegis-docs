...
Tool availability is determined entirely by which tool servers are declared in `aegis-config.yaml` (see [MCP Tool Routing](/docs/architecture/tool-routing)). No tool configuration is required inside the agent container — `bootstrap.py` never sees or manages tool schemas.
