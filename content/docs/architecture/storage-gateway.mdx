...
The FUSE daemon acts as a **gRPC FSAL proxy**: every POSIX system call from an agent container flows through the kernel FUSE layer to the daemon, which forwards the operation to the orchestrator's `FsalService` gRPC endpoint. The orchestrator applies all AegisFSAL security checks and routes the operation to the appropriate `StorageProvider`.
