...
1. An execution starts. The orchestrator calls `FuseMountService.Mount(execution_id, volume_id, tenant_id)` on the FUSE daemon.
2. The FUSE daemon creates a FUSE mountpoint at `<mount_prefix>/<volume_id>` (e.g., `/tmp/aegis-fuse/<volume_id>`).
3. The daemon returns the host mount path in the `MountResponse`.
4. The orchestrator creates the agent container with a bind mount from the host FUSE path to the container's `/workspace` (or the manifest-declared `mount_path`).
5. All POSIX operations from the container flow through the kernel FUSE layer to the daemon, which proxies them via gRPC to the orchestrator's `FsalService`.
6. When the execution ends, the orchestrator calls `FuseMountService.Unmount(execution_id, volume_id)`. The daemon unmounts the FUSE filesystem and cleans up.
...
1. The FUSE daemon mounts a FUSE filesystem at a host path (e.g., `/tmp/aegis-fuse/<volume_id>`).
2. The container is started with a bind mount from the host FUSE mountpoint into the container's `/workspace`.
3. All POSIX operations from the container flow through the kernel's FUSE layer to the FUSE daemon, which proxies them via gRPC to `AegisFSAL`.
...
This is safe for the common case of single-agent-per-volume. For multi-agent coordination (swarms), use the [ResourceLock](/docs/workflows/swarms#resource-locking) mechanism provided by the swarm coordination context instead of POSIX locks.
...
Storage events originating from agent FSAL calls carry an `execution_id` referencing the agent execution.
Events from ContainerStep FUSE filesystem reads carry a `workflow_execution_id` instead.
The `execution_id` and `workflow_execution_id` fields are mutually exclusive — exactly one is always set.
:::
...
## Post-Execution File Retrieval

After an execution completes, files from the execution's workspace volume remain accessible through two interfaces:

- **`aegis.execution.file` MCP tool** — retrieves individual files by path from a completed execution's workspace. Text files are returned inline in the tool result for direct LLM consumption. Binary files (images, PDFs, archives, etc.) return a `download_url` pointing to the REST endpoint for out-of-band retrieval by the client application.
- **`GET /v1/executions/:id/files/*path` REST endpoint** — streams the raw file bytes directly from the storage backend. Used by the `download_url` in binary file tool results and available for direct HTTP access by client applications.

Both interfaces read directly from the storage backend (SeaweedFS or the active `StorageProvider`) using the `AegisFSAL`. No FUSE mount or NFS export is needed post-execution — the volume data persists in SeaweedFS after the container stops and the NFS export is removed (volume state `Detached`).

File content type detection uses a combination of file extension mapping and content sniffing to determine whether a file is text or binary. Text files have their contents included inline in the MCP tool result; binary files are served via the REST endpoint to avoid polluting the LLM context with base64-encoded data.

Files remain accessible until the volume is garbage-collected:

- **Ephemeral volumes** (ownership `Execution` or `WorkflowExecution`): available until the volume's TTL expires (default 24 hours after execution completes).
- **Persistent volumes**: available indefinitely until explicitly deleted.

For the architectural decision behind binary file download support, see ADR-108.
