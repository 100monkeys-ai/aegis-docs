...
StorageProvider trait (via StorageRouter)
  ...
  └── SmcpStorageProvider (Remote Node execution coordination)
```
...
`Failed` volumes are surfaced in `aegis volume list` output and may be retried with `aegis volume delete <id> --force`.
