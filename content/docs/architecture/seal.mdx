...
| `1005` | `UNKNOWN_SESSION` | `sub` claim does not match any active `SealSession` |
...
| `2004` | `COMMAND_NOT_ALLOWED` | Command or subcommand not in `subcommand_allowlist` |
...
| `3001` | `SCOPE_NOT_FOUND` | `requested_context` does not name a known `SecurityContext` |
...
SEAL SDKs are available for Python and TypeScript. In AEGIS, `bootstrap.py` uses the Python SDK internally — you do not call SEAL directly when writing agents. The SDK reference below is relevant when integrating SEAL into your own orchestration layer.
