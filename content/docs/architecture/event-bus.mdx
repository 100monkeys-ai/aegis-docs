...
The channel has a ring‑buffer capacity of 1000 events. If subscribers are slow and the buffer fills, the oldest events are dropped. In Phase 1, this is acceptable because all subscribers are in-process and fast. Future persistent event streaming (e.g., Redpanda) will replace this for durability.
...
Storage events support dual execution context: agent FSAL calls set `execution_id`; ContainerStep FUSE filesystem reads set `workflow_execution_id` instead. The two fields are mutually exclusive — exactly one is always set.

### MCP Tool Events

Published by the `SealMiddleware` and tool routing layer:
...
Published by `AttestationService` and `SealMiddleware`:
...
Published to the `"stimulus"` topic by the Stimulus-Response routing pipeline. Consumed by the Cortex learning pipeline for pattern analysis.

| Event | Fields | When |
|---|---|---|
| `StimulusReceived` | `stimulus_id`, `source`, `received_at` | Stimulus ingested via any endpoint |
| `StimulusClassified` | `stimulus_id`, `workflow_id`, `confidence`, `routing_mode`, `classified_at` | Routing succeeds — `routing_mode` is `"Deterministic"` (Stage 1 direct route match) or `"LlmClassified"` (Stage 2 RouterAgent fallback) |
| `StimulusRejected` | `stimulus_id`, `reason`, `rejected_at` | Routing fails (low confidence, no router, HMAC invalid, duplicate) |
| `ClassificationFailed` | `stimulus_id`, `error`, `failed_at` | RouterAgent errors during classification |
...
| `Audit logger` | All SEAL, Policy, Command events | Structured log entries with signatures |
...
| `Cortex learning pipeline` | All Stimulus events | Pattern analysis for routing confidence tuning |
...
tokio::spawn(async move {
    while let Ok(event) = rx.recv().await {
        match event {
            ...
            DomainEvent::Seal(SealEvent::PolicyViolationBlocked { agent_id, tool_name, .. }) => {
...
**No cross-node delivery**: Each orchestrator node has its own event bus. In multi-node deployments (future), events are not automatically shared across nodes. Cross-node event streaming via a message broker (e.g., NATS, Redpanda) is planned for post-Phase 1.
