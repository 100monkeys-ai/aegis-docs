...
- **Multi-Tenant Isolation:** Enterprise tenants operate within isolated boundaries via dedicated Keycloak realms (`tenant-{slug}`). Consumer users share the `zaru-consumer` Keycloak realm but each receive a dedicated per-user tenant (slug format `u-{uuid}`) at signup. The orchestrator derives the consumer's `tenant_id` from a custom JWT claim rather than from the realm slug, ensuring full data isolation between individual consumer users.
- **Authorization Checks:** The orchestrator's gRPC and HTTP APIs intercept requests using the `GrpcIamAuthInterceptor` to validate Bearer JWTs via a cached JWKS endpoint before allowing requests to proceed.
