...
| **Keycloak** | Token exchange for human-delegated auth. Management endpoint authentication via `aegis-system` realm JWTs. |
...
| Threat | Mitigation |
|---|---|
| Prompt injection via workflow authoring | Management API requires `AegisRole::Operator` JWT. Standard agents cannot register workflows. |
