...
| Policy engine                | Cedar                                                                                       |
| Platform deployment          | Podman Kube YAML + Makefile ([aegis-deploy](https://github.com/100monkeys-ai/aegis-deploy)) |
| Reverse proxy                | Caddy (automatic TLS via ACME)                                                              |
| Log aggregation              | Loki + Promtail                                                                             |
| Distributed tracing          | otelcol-contrib (OTLP receiver) → Jaeger                                                    |
...
| SEAL security protocol                                                                     | [SEAL Protocol](/docs/gateway/seal-protocol)                      |
...
| SEAL Tooling Gateway — macro-tool workflows, ephemeral CLI, credential proxy               | [SEAL Gateway](/docs/gateway/overview)                            |
| Stimulus-Response Routing — two-stage hybrid routing pipeline for external event ingestion | [Stimulus-Response Routing](/docs/architecture/stimulus-response) |
| Edge Mode — user-installed daemons, Relay Coordinator, fleet operations                    | [Edge Mode Overview](/docs/concepts/edge-mode-overview)           |

---

## Edge Daemon Layer

AEGIS supports a fourth deployment topology beyond Controller / Worker / Hybrid: **Edge Daemons** running on user-owned hosts (laptops, home servers, work VMs). They are not orchestrator peers — they do not appear in `healthy_workers()` and cannot receive `ForwardExecution`. Instead they accept MCP tool calls dispatched specifically to them via the `EdgeRouter`.
...
└────────┬───────────┘
         ...
         ▼
┌────────────────────┐         ┌──────────────────────┐
│ AEGIS Controller   │ ◀─────▶ │ Relay Coordinator    │   (SaaS only)
│ (Hybrid in OSS)    │         │ relay.example.com    │
└────────┬───────────┘         └──────────┬───────────┘
         │                                 │
         │                                 │ gRPC bidi stream
         ▼                                 ▼  (outbound from daemon)
┌────────────────────┐         ┌──────────────────────┐
│ Worker pool        │         │ Edge daemons         │
│ (forwarded exec)   │         │ (user-owned hosts)   │
└────────────────────┘         └──────────────────────┘
```

The Relay Coordinator is the SaaS multi-tenant deployment of `NodeClusterService` exposed at `relay.myzaru.com:443`. In OSS deployments edge handling co-locates with the controller — no separate process required.
