...
Rate limits from the root user's `ZaruTier` `SecurityContext` apply collectively across all child tool calls within a swarm.
...
Every SMCP operation publishes a domain event for the audit trail. These are consumed by the Cortex, The Synapse, and compliance reporting:
